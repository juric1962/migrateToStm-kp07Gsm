...
union
            {                        // структура байта управления
             ...
             struct
                   {
                   unsigned char nomer_ty      :3;       // 5 - взвод флага для станции, номер ту 1-4, 7-cbros tu
...
struct struct_ts_mb ts0_mb,ts1_mb,ts2_mb,ts3_mb,ts4_mb,ts5_mb,ts6_mb,ts7_mb,ts14_mb,ts15_mb;
...
{
  if(sel_modul!=0)return;                 // в режиме МКД нет управления !
  if (num==1) {TU1_ON;}
  if (num==2) {TU2_ON;}
...
{
  if(sel_modul!=0)return;
if (num==1)  {TU1_ON;cnt_tu1=bv+1;}
if (num==2)  {TU2_ON;cnt_tu2=bv+1;}
...
void start_ts_mb(unsigned char num,struct struct_ts_mb *ts)
{
  ...
  if(modbus_mem1[AD_TS] & maska_ts)ts->tek_tc=0;else ts->tek_tc=1;
  ts->old_tc=ts->tek_tc;
  
 
  
                if( (num==4)  && (sel_modul!=0) ) if(ts->tek_tc==0)flag_set_akb=0;else flag_set_akb=1; // сеть МКД модуль
               if( (num==3)  && (sel_modul==0) )  if(ts->tek_tc==0)flag_set_akb=0;else flag_set_akb=1; // сеть МТС модуль
       ...
       if(ts->tek_tc==ts->old_tc)
        {
            ...
            if(ts->count_tc==0)
              ...
              */
               ...
               if (uprava.bit.wr_ch)insert_zapis_tc(num,ZP_CH_TC);  // запись в архив изменений ТС
...
opros_ts_mb(15,&ts15_mb);
 ...
 for (ivx=0;ivx<cc;ivx++)
 {
   ...
   uprava.config=c_config_tc[ivx];   
    switch(ivx)
      {
        case 0: opros_ts_mb(ivx,&ts0_mb);break;
        case 1: opros_ts_mb(ivx,&ts1_mb);break;
        case 2: opros_ts_mb(ivx,&ts2_mb);break;
        case 3: opros_ts_mb(ivx,&ts3_mb);break;
        case 4: opros_ts_mb(ivx,&ts4_mb);break;//акб сеть  
        case 5: opros_ts_mb(ivx,&ts5_mb);break;
        case 6: opros_ts_mb(ivx,&ts6_mb);break;
        case 7: opros_ts_mb(ivx,&ts7_mb);break;
      }
 }
...
//unsigned char ivx,j;//Индекс входа, временный байт
 ...
 if ( *(unsigned int*)&c_config_tc[0x16] !=0x55aa) return;
...
//Блок контроля ТИТ====================  
 ...
 for (ivx=0;ivx<2;ivx++)       // 
 {
  ...
  if (sec_ttt[ivx]==real_time.r_sec){run_1=run_1<<1;continue;} //Идет отработка паузы. След.вх.
  ...
  if ( analog_uprava.map.en_alarm_low !=1) { goto control_max_tit;} // запрещено сигнализация о авариях
...
//Контроль Уmin -----------------
  if ( ((*triger_titmin   & run_1)==0) &&                                //ТУ еще не выдавалоь
     ...
     {sec_ttt[ivx]=real_time.r_sec;  k_a_tit[ivx]=(k_a_tit[ivx]+1 ) & 0xf; 
      if (k_a_tit[ivx]==(3+ analog_ust_1.map.delay)  )       //программированная выдержка авария -> выдатьТУ
      {   
          
          *triger_titmin=*triger_titmin   | run_1;      // установить бит аварии по минимуму
          *triger_titmax=*triger_titmax   & ~run_1;      // сбросить бит аварии по максимуму для разблокировки 
          
         
          
          
          
           if (analog_uprava.map.nomer_ty_low>0  && analog_uprava.map.nomer_ty_low <= 3 )  //Допустимый.NТУ
           
          {
          if(analog_uprava.map.type_ty_low==0) impuls_avatu(analog_uprava.map.nomer_ty_low,analog_ust_1.map.impuls);
          if(analog_uprava.map.type_ty_low==1) set_avatu(analog_uprava.map.nomer_ty_low);
          if(analog_uprava.map.type_ty_low==3) set_avatu(analog_uprava.map.nomer_ty_low);
          if(analog_uprava.map.type_ty_low==2) reset_avatu(analog_uprava.map.nomer_ty_low);  
...
control_max_tit: 
  ...
  if ( analog_uprava.map.en_alarm_hi !=1) goto control_ala;
  if ( ((*triger_titmax   & run_1)==0) &&                                 //ТУ еще не выдавалоь
     ...
     {sec_ttt[ivx]=real_time.r_sec;  k_a_tit[ivx]=(k_a_tit[ivx]+1 ) & 0xf; 
      ...
      {  
           ...
           *triger_titmax=*triger_titmax   | run_1;      // установить бит аварии по максиму
           *triger_titmin=*triger_titmin   & ~run_1;      // сбросить бит аварии по минимуму для разблокировки  
          
          
          
          
            if (analog_uprava.map.nomer_ty_hi>0  && analog_uprava.map.nomer_ty_hi <= 3 )  //Допустимый.NТУ
           
          {
          if(analog_uprava.map.type_ty_hi==0) impuls_avatu(analog_uprava.map.nomer_ty_hi,analog_ust_1.map.impuls);
          if(analog_uprava.map.type_ty_hi==1) set_avatu(analog_uprava.map.nomer_ty_hi);
          if(analog_uprava.map.type_ty_hi==3) set_avatu(analog_uprava.map.nomer_ty_hi);
          if(analog_uprava.map.type_ty_hi==2) reset_avatu(analog_uprava.map.nomer_ty_hi);  
 ...
 control_ala: 
  ...
  if ( k_a_tit[ivx]== (3+ analog_ust_1.map.delay ) &&                                //ТУ  выдавалоь
      ((modbus_mem1[AD_TIT1+ivx])>= (analog_min+analog_ust_2.map.gisteresis) ) && // >= Уmin + гистерезис
      ((modbus_mem1[AD_TIT1+ivx])< (analog_max-analog_ust_2.map.gisteresis)  )     // < Уmax  - гистерезис
     )//Значение в допуске
    { k_a_tit[ivx]=0;
    if(analog_uprava.map.type_ty_low==3) reset_avatu(analog_uprava.map.nomer_ty_low);
    if(analog_uprava.map.type_ty_hi==3)  reset_avatu(analog_uprava.map.nomer_ty_hi);
