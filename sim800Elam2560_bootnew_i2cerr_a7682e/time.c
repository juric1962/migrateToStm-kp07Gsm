...
char  * time_keeper;
 ...
 //   последовательное чтение начина€ с адреса в массив
 ...
 void delay(unsigned int period)
  ...
  for(i=0;i<8;i++)
  {
      if(tt & 0x80)    PORTE=PORTE | SDA;      //+r
    else             PORTE=PORTE & ~SDA;       //+r
     cloc_scl();
     tt=tt<<1;
  }
...
//ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒдд работа с часами
...
//        и удерживаетс€ в 1 нет подтверждени€
//
      if(j==6 ) PORTE=PORTE  | SDA;     //  no ack      //+r
