...
__flash char map_vers[]  =   {'F','K','0','1','.','0','1',':','0','6','0','4','0','7'};
__flash char versia[] = {'V','K','0','1','.','0','1',':','0','2','0','7','0','7'};
...
//
      if(j==(num-1 )) PORTE=PORTE  | SDA;     //  no ack        //+r
     else PORTE=PORTE & ~SDA;     //  ack                       //+r
 //
...
void WrArrayToFlesh(unsigned int adres_flesh,  unsigned char *adres_ozu, unsigned int num,unsigned char flag,unsigned char znach)   //caienu eiioeao?aoee ei ai oeao
{
	unsigned int i,nums; 
	...
	if((adres_flesh+num)>32767)//?iau ia caoa?aou aaiiua, ii?ao yoi e ia iaai anee ii oeeeo ienaou
		return;
	nums = 64-offset;
	if(num<nums)
		nums = num;
	...
	{       //delay(8000);
		...
		for (i=0;i<nums;i++)
		{
		 ...
		 if (flag==0)out(*adres_ozu++);else out(znach); 
...
povt_1:            
    ...
    case 14: 
     ...
     case 15:      
               ...
               if(*(unsigned long int*)&buf[0]==9600)sprintf(massiv,"%s","9600");
               else if(*(unsigned long int*)&buf[0]==115200)sprintf(massiv,"%s","115200");
               else sprintf(massiv,"%s","-----");
     ...
     case 19:
         ...
         gmn1:  for(i=0;i<8;i++)
                    {
                    if(j & 0x01)s_port('1');else s_port('0');
                    j=j>>1;
                    }
        ...
        {      
              ...
              case 10: 
                ...
                case 15:
                      ...
                      cifra_long.word=atol((char *)&buf_tx_232[0]);                                                                                      
                      if ((cifra_long.word!=9600)&&(cifra_long.word!=115200))goto bad_com_232;
