...
void  transform_buf(unsigned char *p,unsigned int kol_byte,unsigned int s_rand);
...
__flash char rec_ctrl_ch[]=             {'R','E','C',':','c','t','r','l',' ','c','h'};
...
__flash char req_in_st_contr[]={'R','E','Q','-','<','s','t',' ','c','o','n','t','r'};
...
__flash char ans_out_st_contr[]={'A','N','S','-','>','s','t',' ','c','o','n','t','r'};
...
__flash  unsigned char new_version[]={0x06,0xcd,0x00,0x07,0x00,NO_MODEL,0x00,NO_MAP,DATA_MONTH,DATA_DATA,0x00,GOD};
...
unsigned int proc_config(unsigned char *buf_rx_ppp,unsigned char offset,unsigned int count_rx_ppp);
unsigned char check_ln_conf(unsigned char *buf_rx_ppp,unsigned char offset,unsigned int count_rx_ppp);
...
unsigned char proc_udp_data(unsigned char *buf_rx_ppp,unsigned int count_rx_ppp);
...
void WrArrayToFlesh(unsigned int adres_flesh,  unsigned char *adres_ozu, unsigned int num,unsigned char flag,unsigned char znach);

void send_err485(unsigned char port,unsigned char err,unsigned char id,unsigned int dst);
       ...
       if((pap_act_buf==SCR)&&(fl_pap.act_pap_end==1)&&(fl_lcp.act_lcp_end==1)&&(fl_ipcp.act_ipcp_end==1)&&(fl_ip.act_ip_end==1))
        {        
                 ...
                 if((temp.bytes[0]==0)||(temp.bytes[0]>MAX_VOL_US_NAME)) buf_tx_232[7]=0;
                    else {buf_tx_232[7]=temp.bytes[0];RdFromFleshToArr(A_C_GPRS+temp1+2,&buf_tx_232[8],temp.bytes[0]);}
               
                 if((temp.bytes[1]==0)||(temp.bytes[1]>MAX_VOL_PSW)) buf_tx_232[8+buf_tx_232[7]]=0;
                    else {buf_tx_232[8+buf_tx_232[7]]=temp.bytes[1];RdFromFleshToArr(A_C_GPRS+temp1+temp.bytes[0]+3,&buf_tx_232[9+buf_tx_232[7]],temp.bytes[1]);}
                 }
                 else   //!!!!!!!!!!dobavka
                 {
                 ...
                 if((temp.bytes[0]==0)||(temp.bytes[0]>MAX_VOL_US_NAME)) buf_tx_232[7]=0;
                    else {buf_tx_232[7]=temp.bytes[0];RdFromFleshToArr(A_CR_GPRS+temp1+2,&buf_tx_232[8],temp.bytes[0]);}
               
                 if((temp.bytes[1]==0)||(temp.bytes[1]>MAX_VOL_PSW)) buf_tx_232[8+buf_tx_232[7]]=0;
                    else {buf_tx_232[8+buf_tx_232[7]]=temp.bytes[1];RdFromFleshToArr(A_CR_GPRS+temp1+temp.bytes[0]+3,&buf_tx_232[9+buf_tx_232[7]],temp.bytes[1]);}
                 }
                  //!!!!!!!!!!dobavka
...
void monitor_event_IPCP(void)
 ...
 unsigned char proc_ppp_packet(unsigned char *buf_rx_ppp,unsigned int count_rx_ppp)
 {                
    unsigned int i;
...
return(0);
 ...
 unsigned char proc_option(unsigned char *buf_rx_ppp,unsigned int ind,unsigned int count_rx_opt,unsigned char port)
 {
                 ...
                 case KOD_OP_LIFE:   
                                      ...
                                      else
                                      {
                                        ...
                                        else 
                                        {
                                          ...
                                          if((life_cl==*(unsigned int*)&buf_rx_ppp[TR_SRC]) || (life_start==1))
                                          {
                                            switch(port)
                                                {
                                                case PORT485_1:
 ...
 */
  ...
  unsigned char proc_udp_data(unsigned char *buf_rx_ppp,unsigned int count_rx_ppp)
        ...
        unsigned int overall;
               ...
               transform_buf(&buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_LEN]-7,*(unsigned int*)&buf_rx_ppp[TR_KOD]);
               ...
               *(unsigned int*)&buf_rx_ppp[TR_CRC]=0;
               ...
               //проверка crc
                if(crc!=crc_m1(&buf_rx_ppp[TR_SRC],*(unsigned int*)&buf_rx_ppp[TR_LEN],0xffff))return(0);         
               ...
               if(*(unsigned int*)&buf_rx_ppp[TR_DST]!=num_self)return(0); 
...
//                  send_info(sizeof(prov2),prov2,0,0);
                  ...
                  */
                   ...
                   if(*(unsigned int*)&buf_rx_ppp[TR_SRC]!=NUM_SRV_LINK)return(0);
//                  if(buf_rx_ppp[TR_ID]!=ID_CC)return(0);
                ...
                if(buf_rx_ppp[offset+SL_LS_PROT]==PROT_SL_LS)
                  {
                    ...
                    if(*(unsigned int*)&buf_rx_ppp[offset+SL_LS_PROT+2]!=L_OTV_CNTR_CL)return(0);
                    if(*(unsigned int*)&buf_rx_ppp[offset+SL_LS_PROT+4]!=num_seq_cl)return(0);
                  ...
                  switch(buf_rx_ppp[offset+C1_PORT])
                      {
                       case PORT485_1:
                            ...
                            if(length<11) 
                                  {
                                   ...
                                   send_err485(NUM_RS485_1,RS_DATA_ERR,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                   return(0);
                                  }                 
                             ...
                             if(length> LN_BUF_485_1+10) 
                                ...
                                // можно отправиь сообщение об ошибке "слишком длинные данные для приложения"
                                   send_info(sizeof(ans_out_485_1),ans_out_485_1,1,buf_rx_ppp[TR_ID]);
                                   send_err485(NUM_RS485_1,RS_OVER_BUF_TX,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                   return(0);
                                  } 
                            ...
                            // то можно послать служебное сообщение
                                 ...
                                 {
                                   send_info(sizeof(ans_out_485_1),ans_out_485_1,1,buf_rx_ppp[TR_ID]);
                                   send_err485(NUM_RS485_1,RS_NO_CONT,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                   return(0);
                                  } 
                            ...
                            if(Appl_RS485_1.fl_data_buf==TRUE)
                               ...
                               send_info(sizeof(ans_out_485_1),ans_out_485_1,1,buf_rx_ppp[TR_ID]);
                               send_err485(NUM_RS485_1,RS_BUSY,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                               break;
                               }
                               ...
                               {// включен тестовый режим!!!
                                 send_info(sizeof(ans_out_485_1),ans_out_485_1,1,buf_rx_ppp[TR_ID]);
                                 send_err485(NUM_RS485_1,RS_TEST,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                 return(0);
                               }
                                ...
                                if(length_head>MIN_TR_HEAD)
                                  {
                                    i=proc_option(buf_rx_ppp,TR_OP_DATA,length_head-MIN_TR_HEAD,PORT485_1);
                                    
                                    
                                    if(i==3)//время жизни истекло
                                    {  
                                      send_no_sinc(buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC],OUT_LIFE);
                                      return(0);
                                    }
                                    
                                    if(i==4) //операция недоступна
                                      {
                                      send_no_sinc(buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC],DES_OPERATE);
                                      return(0);
                                      }
                               ...
                               if(fl_485_1.busy==1)Appl_RS485_1.fl_data_buf=TRUE;
                               else fl_485_1.tx=1;
                               Appl_RS485_1.dst_buf=*(unsigned int*)&buf_rx_ppp[TR_SRC];
                               Appl_RS485_1.id_buf=buf_rx_ppp[TR_ID];
                               
                               for(i=0;i<8;i++)Appl_RS485_1.cont_buf[i]=buf_rx_ppp[offset+C1_CONT+i];
                               
                               Appl_RS485_1.ln_data_buf=length-10;
                               for(i=0;i<Appl_RS485_1.ln_data_buf;i++)Appl_RS485_1_data_buf[i]=buf_rx_ppp[offset+C1_DATA_RS+i];
                       ...
                       case PORT485_2:
                                ...
                                if(length<11) 
                                  {
                                   ...
                                   send_err485(NUM_RS485_2,RS_DATA_ERR,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                   return(0);
                                  }                 
                             ...
                             if(length> LN_BUF_485_2+10) 
                                ...
                                // можно отправиь сообщение об ошибке "слишком длинные данные для приложения"
                                   send_info(sizeof(ans_out_485_2),ans_out_485_2,1,buf_rx_ppp[TR_ID]);
                                   send_err485(NUM_RS485_2,RS_OVER_BUF_TX,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                   return(0);
                                  } 
                            ...
                            // то можно послать служебное сообщение
                             if (check_cont_485_1 (&buf_rx_ppp[offset+C1_CONT])==1)
                                 {
                                   send_info(sizeof(ans_out_485_2),ans_out_485_2,1,buf_rx_ppp[TR_ID]);
                                   send_err485(NUM_RS485_2,RS_NO_CONT,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                   return(0);
                                  } 
                            ...
                            if(Appl_RS485_2.fl_data_buf==TRUE)
                               ...
                               // то можно послать служебное сообщение что буфер переполнен
                               send_info(sizeof(ans_out_485_2),ans_out_485_2,1,buf_rx_ppp[TR_ID]);
                               send_err485(NUM_RS485_2,RS_BUSY,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                               break;
                               }
                               ...
                               {// включен тестовый режим!!!
                                  send_info(sizeof(ans_out_485_2),ans_out_485_2,1,buf_rx_ppp[TR_ID]);
                                 send_err485(NUM_RS485_2,RS_TEST,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                 return(0);
                               }
                               ...
                               Appl_RS485_2.pre_tx=0;
                                if(length_head>MIN_TR_HEAD)
                                  {
                                    i=proc_option(buf_rx_ppp,TR_OP_DATA,length_head-MIN_TR_HEAD,PORT485_2);
                               ...
                               if(fl_485_2.busy==1)Appl_RS485_2.fl_data_buf=TRUE;
                               else fl_485_2.tx=1;
                               Appl_RS485_2.dst_buf=*(unsigned int*)&buf_rx_ppp[TR_SRC];
                               Appl_RS485_2.id_buf=buf_rx_ppp[TR_ID];
                               for(i=0;i<8;i++)Appl_RS485_2.cont_buf[i]=buf_rx_ppp[offset+C1_CONT+i];
                               Appl_RS485_2.ln_data_buf=length-10;
                               for(i=0;i<Appl_RS485_2.ln_data_buf;i++)Appl_RS485_2_data_buf[i]=buf_rx_ppp[offset+C1_DATA_RS+i];
                     ...
                     case PORT232_2:
                                ...
                                if(length<11) 
                                  {
                                    ...
                                    send_err485(NUM_RS232_2,RS_DATA_ERR,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                   return(0);
                                  }                 
                             ...
                             if(length> LN_BUF_232_2+10) 
                                ...
                                // можно отправиь сообщение об ошибке "слишком длинные данные для приложения"
                                   send_info(sizeof(ans_out_232),ans_out_232,1,buf_rx_ppp[TR_ID]);
                                   send_err485(NUM_RS232_2,RS_OVER_BUF_TX,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                   return(0);
                                  } 
                            ...
                            // то можно послать служебное сообщение
                             if (check_cont_485_1 (&buf_rx_ppp[offset+C1_CONT])==1)
                                 {
                                   send_info(sizeof(ans_out_232),ans_out_232,1,buf_rx_ppp[TR_ID]);
                                   send_err485(NUM_RS232_2,RS_NO_CONT,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                   return(0);
                                  } 
                            ...
                            if(Appl_RS232_2.fl_data_buf==TRUE)
                               ...
                               // то можно послать служебное сообщение что буфер переполнен
                               send_info(sizeof(ans_out_232),ans_out_232,1,buf_rx_ppp[TR_ID]);
                                 send_err485(NUM_RS232_2,RS_BUSY,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                               break;
                               }
                               ...
                               {// включен тестовый режим!!!
                                send_info(sizeof(ans_out_232),ans_out_232,1,buf_rx_ppp[TR_ID]);
                                 send_err485(NUM_RS232_2,RS_TEST,buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC]);
                                 return(0);
                               }
                       ...
                       case PORT_SEQ:
                           ...
                           // if(*(unsigned int*)&buf_rx_ppp[TR_SRC]!=num_seq_cl)return(0);
                            ...
                            if(((buf_rx_ppp[TR_V]>>4)&0x03)==KVIT)
                                {
                                if(*(unsigned int*)&buf_rx_ppp[TR_SRC]!=num_seq_cl)return(0);   
                                if(buf_rx_ppp[offset+C1_DATA]!=KVITOK_SEQ)return(0);
                                if(length!=5)return(0);
                                if(*(unsigned int*)&buf_rx_ppp[offset+C1_DATA+1]!=L_KVITOK)return(0);
                            ...
                            if(((buf_rx_ppp[TR_V]>>4)&0x03)==ZAPR)
                               ...
                               if(length!=5)return(0);
                               if(*(unsigned int*)&buf_rx_ppp[offset+C1_DATA+1]!=L_ZAPR_TC)return(0);
                               ...
                               Obj_ppp_tx.type_pac=OTV;     //тип пакета
                               Obj_ppp_tx.num_src=num_self; // номер отправителя
                               Obj_ppp_tx.num_dst=*(unsigned int*)&buf_rx_ppp[TR_SRC]; // номер получателя
                                ...
                                *(unsigned long int*)&buf_tx_232[TR_OP_DATA+C1_DATA+3]=burst_ds_r();
                              
                                
                                
                               if(struct_tcc1.type==TC_SHL) buf_tx_232[TR_OP_DATA+C1_DATA+8]=state_seq;
                                   else buf_tx_232[TR_OP_DATA+C1_DATA+7]=DES_SEQ;
                               
                               if(Appl_seq_des!=0) buf_tx_232[TR_OP_DATA+C1_DATA+7]=DES_SEQ;
                                
                               if((struct_tcc1.type==TC_SHL)||(struct_tcc1.type==TC_SUHOI))buf_tx_232[TR_OP_DATA+C1_DATA+8]=sv1.tc & 0x01;
                               else {buf_tx_232[TR_OP_DATA+C1_DATA+7]=DES_SEQ; buf_tx_232[TR_OP_DATA+C1_DATA+8]=DES_SEQ;}
                               
                               if((struct_tcc2.type==TC_SHL)||(struct_tcc2.type==TC_SUHOI))buf_tx_232[TR_OP_DATA+C1_DATA+9]=sv2.tc & 0x01;
                                 else buf_tx_232[TR_OP_DATA+C1_DATA+9]=DES_SEQ;
                       ...
                       case PORT_SYS:
                        ...
                        case PORT_CONF:
                            ...
                            {
                                 if(length>5)return(0);
                                 if(*(unsigned int*)&buf_rx_ppp[offset+C1_DATA+1]!=L_CONF_LOG)return(0);
                                 if((buf_rx_ppp[offset+C1_DATA] & 0x80)==0x80)
                                    ...
                                    __watchdog_reset();
                                    point_log_buf=0;
                                    *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+1]=L_CONF_LOG+1;
                                  ...
                                  //  ii=0;
                                    ...
                                    {
                                    //чтение 
                                    RdFromFleshToArr(BEG_BUF_LOG,&buf_tx_232[TR_OP_DATA+C1_DATA+4],(L_LOG*6));
                                    *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+1]=L_CONF_LOG+(L_LOG*6)+1;
  ...
  otv_conf:                        
                                ...
                                if((fl_rewrite.ip==1)||(fl_rewrite.udp==1)||(fl_rewrite.num_self==1))
                                   {
                                    ...
                                    if(fl_rewrite.num_self==1)num_self=*(unsigned int*)&buf_rx_ppp[OFS_NUM];
                                    if(fl_rewrite.udp==1)port_udp=*(unsigned int*)&buf_rx_ppp[OFS_PORT];
                                    if(fl_rewrite.ip==1)
                                          {
                       ...
                       case PORT_PROG:
                            ...
                            if(length_head==MIN_TR_HEAD)return(0);     
                            if (proc_option(buf_rx_ppp,TR_OP_DATA,length_head-MIN_TR_HEAD,PORT_PROG)!=1)return(0); 
                             
                            if(length!=5+L_COM_CODE_MEM_Z)return(0);        
                            if((buf_rx_ppp[offset+C1_PORT+1])!=COM_CODE_MEM)return(0);
                            if((*(unsigned int*)&buf_rx_ppp[offset+C1_PORT+2])!=L_COM_CODE_MEM_Z)return(0);
...
// прошивка не соответствует устройству !!  25 12 2021
 device_bad:                                 
                               send_no_sinc(buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC],NO_SINC_PG);
                               return(0);
                               }
 ...
 //                             send_info(sizeof(gluk),gluk,1,22);
                              ...
                              fl_pg_out.end_pg=0;
                              fl_pg_out.cor_pg=0;
                              send_no_sinc(buf_rx_ppp[TR_ID],*(unsigned int*)&buf_rx_ppp[TR_SRC],OVER_PG);
                              return(0);
                              }
   ...
   //                           send_info(sizeof(gluk),gluk,1,23);
                              ...
                              if(fl_pg_out.cor_pg==1)
                                 {
                                 fl_pg_out.cor_pg=0;
                                   // проверка на соответствие прошивки в конце страницы должен быть код 0x55aa                      
  //              if ( ( cnt_pg==0) &&     ( *(unsigned int*)&buf_rx_ppp[offset+C1_PORT+4 + 0xFE] != 0x55AA ) ) goto device_bad;   
                   if ( ( cnt_pg==0) &&    ( buf_rx_ppp[offset+C1_PORT+4 + 0xFF] != VERSION_API ) ) goto device_bad;                
   ...
   //                              send_info(sizeof(gluk),gluk,1,24); 
                                  ...
                                  //произвести запись страницы
                                      ...
                                      if( cnt_pg >=256) RAMPZ=3; 
                                      else RAMPZ=2;                              // 18 05 2022 
 ...
 //                                       send_info(sizeof(gluk),gluk,1,26);
                                       ...
                                       i=0;
                                       for (cnt_pg_i=0; cnt_pg_i<total_kol_page; cnt_pg_i++)  // слово !!!
                                           {
                                            array_in_232=&buf_rx_ppp[offset+C1_PORT+4];
                                             if( cnt_pg_i >=256) RAMPZ=3; 
                                              else RAMPZ=2;                              // 18 05 2022 
  ...
  //                              send_info(sizeof(gluk),gluk,1,29);
                                ...
                                if(reboot_byte!=0x55)
                                 {
                                 if(fl_ip.act_ip_end!=1){fl_pg_out.send_crc=0; reboot_byte=0; return(0);}
                                 }
    ...
    //                            send_info(sizeof(gluk),gluk,1,30);
                               ...
                               *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_PROT+13]=L_COM_CODE_MEM_A;
                               ...
                               *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_PROT+16]=cnt_pg; //OK 
                               ...
                               Obj_ppp_tx.p_data=&buf_tx_232[TR_OP_DATA+C1_PROT+10];  //указатель буфера данных
  ...
  //                               send_info(sizeof(gluk),gluk,1,31);
                               ...
                               if(reboot_byte==0x55)
                                 ...
                                 //  *(unsigned long int*)&buf_rx_ppp[0]=burst_ds_r();
                                   ...
                                   *(unsigned int*)&buf_rx_ppp[L_TRAF-2]=crc_m1(&buf_rx_ppp[0],L_TRAF-2,0xffff);
              ...
              case PORT_MBUS:
                            ...
                            /////07 11 2019 процедура проверки пакета на модбас не имеет смысла
                             ...
                             // crc=swap(*(unsigned int*)&buf_rx_ppp[offset+C1_DATA+10]);//здесь временно старт адрес 
                             // kol=swap(*(unsigned int*)&buf_rx_ppp[offset+C1_DATA+12]); //здесь временно количество регистров
                         

                              // Инициализация указателей
  mobus.point_out=& buf_tx_232[TR_OP_DATA+C1_DATA];        // выходной буфер
   mobus.point_in = & buf_rx_ppp[offset+C1_DATA+8];         // входной буфер с модбас пакетом
   mobus.index_elam =0;                                     // индекс очередного инкапсулированного елам пакета
   mobus.flag_elam=0;
...
//
next_elam:
            if ( (  * (  mobus.point_in +  mobus.index_elam) & 0xf8) !=0xf8) goto ne_elam;
             mobus.flag_elam=1;
            switch (* ( mobus.point_in +  mobus.index_elam + 2))
                ...
                case 16:  mobus.size_of_pac=8 + *( mobus.point_in +  mobus.index_elam + 7);break;
                default : {return(0);}
                }
                if ( mobus.count_in_elam < ( mobus.index_elam +  mobus.size_of_pac + 2) )  {goto FinitaPerformence;}
               
               
// проверка фрагмента на CRC
 mobus.modbus_crc_new=crc_m1(mobus.point_in + mobus.index_elam, mobus.size_of_pac , 0xffff);
             if(mobus.modbus_crc_new!=(  (  (unsigned int)( * (mobus.point_in + mobus.index_elam + mobus.size_of_pac) )<<8)
                             + (   unsigned int)( * (mobus.point_in + mobus.index_elam+ mobus.size_of_pac + 1) ) )  )     {return(0);}
...
ne_elam:
              if ( mobus.flag_elam==1)  {return(0);}   // выход с ошибкой 
              // это одиночный пакет
 ...
 mobus.modbus_crc_new=crc_m1(mobus.point_in, mobus.count_in_elam-2,0xffff);
             ...
             if(mobus.modbus_crc_new!=(  (  (unsigned int)( * (mobus.point_in + mobus.count_in_elam-2) )<<8)
                             + (   unsigned int)( * (mobus.point_in + mobus.count_in_elam-1) ) ) ) return(0);
...
// switch обработчик модбас фрагмента                         Начало **************************************                             
    ...
    Performence:                              
                    //          crc=swap(*(unsigned int*)&buf_rx_ppp[offset+C1_DATA+10]);//здесь временно старт адрес 
                    //          kol=swap(*(unsigned int*)&buf_rx_ppp[offset+C1_DATA+12]); //здесь временно количество регистров 
                if (mobus.flag_elam==1) mobus.smest_to_elam=1;
                else mobus.smest_to_elam=0;
                ii= * (mobus.point_in+ mobus.index_elam + mobus.smest_to_elam + 1); // номер команды
                crc =((unsigned int)* (mobus.point_in+ mobus.index_elam + mobus.smest_to_elam+2)<<8)+(unsigned int)* (mobus.point_in + mobus.smest_to_elam + mobus.index_elam+3);
                kol =((unsigned int)* (mobus.point_in+ mobus.smest_to_elam + mobus.index_elam+4)<<8)+(unsigned int)* (mobus.point_in + mobus.smest_to_elam + mobus.index_elam+5);
                           ...
                           switch(ii)
                               {
                                 case 1:
                                   ...
                                   if(mobus.overall > VOL_TX_PPP ) return(0);  // выход по ошибке
                                   // ожидаемое количество байт в выходной буфер
                                   
                                   *mobus.point_out++=*(mobus.point_in + mobus.index_elam);
  if (mobus.flag_elam==1) *mobus.point_out++=*(mobus.point_in+mobus.index_elam+mobus.smest_to_elam);
  *mobus.point_out++=1;
  *mobus.point_out++=1;
  mobus.input_tu=0;
                                    if (crc==MB_TU1)
                                      {
  if(PORTA & IO11) mobus.input_tu=mobus.input_tu | 1;
  if(PORTA & IO12) mobus.input_tu=mobus.input_tu | 2;
                                      }
                                   
                                    if (crc==MB_TU2)
                                      {
                                   if(PORTA & IO12) mobus.input_tu=mobus.input_tu | 1;
                                      }
                                   
                                   *mobus.point_out++=mobus.input_tu;
                                    Obj_ppp_tx.l_data=4 + mobus.smest_to_elam; 
                                   
                              //  *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+Obj_ppp_tx.l_data]=crc_m1(&buf_tx_232[TR_OP_DATA+C1_DATA],Obj_ppp_tx.l_data,0xffff);
                                   mobus.modbus_crc_new=crc_m1(mobus.point_out-Obj_ppp_tx.l_data, Obj_ppp_tx.l_data, 0xffff); 
                                   *mobus.point_out++=mobus.modbus_crc_new>>8;
                                   *mobus.point_out++=mobus.modbus_crc_new;
                                    mobus.count_out_elam+=Obj_ppp_tx.l_data + 2;   // плюс контрольная сумма
                                   ...
                                   /*  старый обработчик
                                      ...
                                      if (crc==MB_TU1)
                                      {
                                        if(PORTA & IO11) buf_tx_232[TR_OP_DATA+C1_DATA+3]=buf_tx_232[TR_OP_DATA+C1_DATA+3] | 0x01;
                                        if(PORTA & IO12) buf_tx_232[TR_OP_DATA+C1_DATA+3]=buf_tx_232[TR_OP_DATA+C1_DATA+3] | 0x02;
                                      }
                                      
                                      if (crc==MB_TU2)
                                      {
                                        if(PORTA & IO12) buf_tx_232[TR_OP_DATA+C1_DATA+3]=buf_tx_232[TR_OP_DATA+C1_DATA+3] | 0x01;
                                      }
                                 ...
                                 case 4:
                                   ...
                                   *mobus.point_out++ = *(mobus.point_in + mobus.index_elam);
                                   if (mobus.flag_elam==1) *mobus.point_out++=*(mobus.point_in+mobus.index_elam + mobus.smest_to_elam);
                                   *mobus.point_out++=ii;
                                   if (mobus.flag_elam==1) *mobus.point_out++=(kol*2) >>8;
                                   *mobus.point_out++=(kol*2);
                                   
                                   
                                    /////buf_tx_232[TR_OP_DATA+C1_DATA+2]=buf_rx_ppp[offset+C1_DATA+13] << 1;//byte_count
                                   ...
                                   *(unsigned int*)(mobus.point_out +(ii<<1))=0;
                                    ...
                                    *mobus.point_out++=(sv1.tc & 0x01) | ((sv2.tc & 0x01)<<1);//  buf_tx_232[TR_OP_DATA+C1_DATA+4]=(sv1.tc & 0x01) | ((sv2.tc & 0x01)<<1);
                                    }
                                    ...
                                    if(((crc+ii)>=BEGIN_SEG7)&&((crc+ii)<=END_SEG7))
                                    {
                                      ...
                                      for(i=0;i<12;i++)buf_tx_232[570+i]=new_version[i];
                                      ...
                                      if((crc+kol-1)<=END_SEG7)
                                          ...
                                          RdFromFleshToArr(A_SEG7+(crc+ii-BEGIN_SEG7)*2,&buf_tx_232[600],(kol-ii)*2);
                                          //for(k=0;k<(kol-ii);k++) *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+3+(ii<<1)+(k<<1)]=swap(*(unsigned int*)&buf_tx_232[600+(k<<1)]);
              ///// for(k=0;k<(kol-ii);k++) *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+3+(ii<<1)+(k<<1)]=*(unsigned int*)&buf_tx_232[600+(k<<1)];
                                       
               for(k=0;k<(kol-ii);k++) *(unsigned int*)(mobus.point_out +(ii<<1)+(k<<1)) =*(unsigned int*)&buf_tx_232[600+(k<<1)];
                                          ii=kol;
                                          }
                                          ...
                                          RdFromFleshToArr(A_SEG7+(crc+ii-BEGIN_SEG7)*2,&buf_tx_232[600],(END_SEG7-(crc+ii)+1)*2);  
                                        //  for(k=0;k<(END_SEG7-(crc+ii)+1);k++) *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+3+(ii<<1)+(k<<1)]=swap(*(unsigned int*)&buf_tx_232[600+(k<<1)]);
            /////  for(k=0;k<(END_SEG7-(crc+ii)+1);k++) *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+3+(ii<<1)+(k<<1)]=*(unsigned int*)&buf_tx_232[600+(k<<1)];
                                          
               for(k=0;k<(END_SEG7-(crc+ii)+1);k++) *(unsigned int*)(mobus.point_out +(ii<<1)+(k<<1))=*(unsigned int*)&buf_tx_232[600+(k<<1)];
                                          ii=END_SEG7-crc;
                                          }
                                       ...
                                       if(((crc+ii)>=BEGIN_SEG9)&&((crc+ii)<=END_SEG9))
                                    {
                                      ...
                                      if((crc+kol-1)<=END_SEG9)
                                          ...
                                          RdFromFleshToArr(A_SEG9+(crc+ii-BEGIN_SEG9)*2,&buf_tx_232[1000],(kol-ii)*2);
                                          for(k=0;k<(kol-ii);k++) *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&buf_tx_232[1000+(k<<1)]);
                                          ii=kol;
                                          }
                                          ...
                                          RdFromFleshToArr(A_SEG9+(crc+ii-BEGIN_SEG9)*2,&buf_tx_232[1000],(END_SEG9-(crc+ii)+1)*2);  
                                          for(k=0;k<(END_SEG9-(crc+ii)+1);k++) *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&buf_tx_232[1000+(k<<1)]);
                                          ii=END_SEG9-crc;
                                          }
                                     ...
                                     if(((crc+ii)>=BEGIN_SEG99)&&((crc+ii)<=END_SEG99))
                                    {
                                      ...
                                      if((crc+kol-1)<=END_SEG99)
                                          ...
                                          RdFromFleshToArr(A_SEG99+(crc+ii-BEGIN_SEG99)*2,&buf_tx_232[1000],(kol-ii)*2);
                                          for(k=0;k<(kol-ii);k++) *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&buf_tx_232[1000+(k<<1)]);
                                          ii=kol;
                                          }
                                          ...
                                          RdFromFleshToArr(A_SEG99+(crc+ii-BEGIN_SEG99)*2,&buf_tx_232[1000],(END_SEG99-(crc+ii)+1)*2);  
                                          for(k=0;k<(END_SEG99-(crc+ii)+1);k++) *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&buf_tx_232[1000+(k<<1)]);
                                          ii=END_SEG99-crc;
                                          }
                                    ...
                                    {
                                      *(unsigned int*)(mobus.point_out+(ii<<1))=swap(modbus_mem1[crc+ii-BEGIN_SEG1]);
                                      if((crc+ii)==MB_IND_DNS)
                                      {
                                        if(modbus_mem1[AD_IND_DNS]==0) *(unsigned int*)(mobus.point_out+(ii<<1))=swap(99);
                                          else *(unsigned int*)(mobus.point_out+(ii<<1))=swap(modbus_mem1[AD_IND_DNS]-1);
                                      }
                                      ////
                                      if((crc+ii)==MB_IND_GZU)
                                      {
                                        if(modbus_mem1[AD_IND_GZU]==0) *(unsigned int*)(mobus.point_out+(ii<<1))=swap(54);
                                          else *(unsigned int*)(mobus.point_out+(ii<<1))=swap(modbus_mem1[AD_IND_GZU]-1);
                                      }
                                      
                                     
                                    }
     
                                    if((crc+ii)==MB_SEC)*(unsigned int*)(mobus.point_out+(ii<<1))=swap(real_time.r_sec);
                                    if((crc+ii)==MB_MIN)*(unsigned int*)(mobus.point_out+(ii<<1))=swap(real_time.r_min);
                                    if((crc+ii)==MB_HR)*(unsigned int*)(mobus.point_out+(ii<<1))=swap(real_time.r_hor);
                                    if((crc+ii)==MB_DAY)*(unsigned int*)(mobus.point_out+(ii<<1))=swap(real_time.r_date);
                                    if((crc+ii)==MB_MONTH)*(unsigned int*)(mobus.point_out+(ii<<1))=swap(real_time.r_month);
                                    if((crc+ii)==MB_YEAR)*(unsigned int*)(mobus.point_out+(ii<<1))=swap(real_time.r_year);
                                    ...
                                    if(((crc+ii)>=BEGIN_SEG3)&&((crc+ii)<=END_SEG3))
                                    {
                                      ...
                                      if((crc+kol-1)<=END_SEG3)
                                          ...
                                          RdFromFleshToArr(A_SEG3+(crc+ii-BEGIN_SEG3)*2,&buf_tx_232[1000],(kol-ii)*2);
                                          for(k=0;k<(kol-ii);k++) *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&buf_tx_232[1000+(k<<1)]);
                                          ii=kol;
                                          }
                                          ...
                                          RdFromFleshToArr(A_SEG3+(crc+ii-BEGIN_SEG3)*2,&buf_tx_232[1000],(END_SEG3-(crc+ii)+1)*2);  
                                          for(k=0;k<(END_SEG3-(crc+ii)+1);k++) *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&buf_tx_232[1000+(k<<1)]);
                                          ii=END_SEG3-crc;
                                          }
                                   ...
                                   // 28 12 2019       RdFromFleshToArr(A_SEG8+(crc+ii-BEGIN_SEG8)*2,&buf_tx_232[1000],(kol-ii)*2);
                                            tc_cfgPoint=(unsigned int*)&buf_tx_232[1000];
                                          for(k=0; k<(kol-ii); k++) *tc_cfgPoint++ = c_config_tc[crc+ii-BEGIN_SEG8 + k];
                                          for(k=0;k<(kol-ii);k++) *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&buf_tx_232[1000+(k<<1)]);
                                          ii=kol;
                                          }
                                     ...
                                     // 28 12 2019 RdFromFleshToArr(A_SEG8+(crc+ii-BEGIN_SEG8)*2,&buf_tx_232[1000],(END_SEG8-(crc+ii)+1)*2); 
                                          tc_cfgPoint=(unsigned int*)&buf_tx_232[1000];
                                          for(k=0; k<(END_SEG8-(crc+ii)+1); k++) *tc_cfgPoint++ = c_config_tc[crc+ii-BEGIN_SEG8 + k];
                                          for(k=0;k<(END_SEG8-(crc+ii)+1);k++) *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&buf_tx_232[1000+(k<<1)]);
                                          ii=END_SEG8-crc;
                                          }
                                    ...
                                    if(((crc+ii)>=BEGIN_SEG6)&&((crc+ii)<=END_SEG6))
                                    {
                                      ...
                                      if((crc+kol-1)<=END_SEG6)
                                         ...
                                         // иначе происходит переполнение массива !!!! 4 10 2021
                                          ...
                                          for(k=0;k<(kol-ii);k++) 
                                          {
                                            RdFromFleshToArr(A_SEG6 + (k + crc + ii - BEGIN_SEG6)*2, &flashka[0], 2);
                                            *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&flashka[0]);
                                          //  *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&buf_tx_232[600+(k<<1)]);
                                          }
                                      ...
                                      if((crc+kol-1)<=END_SEG5)
                                         ...
                                         // иначе происходит переполнение массива !!!! 4 10 2021
                                          for(k=0;k<(kol-ii);k++) 
                                          {
                                            ...
                                            RdFromFleshToArr(A_SEG5 + (k + crc + ii-BEGIN_SEG5)*2,&flashka[0],2);
                                            *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&flashka[0]);
                                           // *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&buf_tx_232[700+(k<<1)]);
                                          }
                                          ii=kol;
                                          }
                                          else
                                     ...
                                     if(((crc+ii)>=BEGIN_SEG55)&&((crc+ii)<=END_SEG55))
                                    {
                                      ...
                                      if((crc+kol-1)<=END_SEG55)
                                          {
                                         // RdFromFleshToArr(STRUCT_US +(crc+ii-BEGIN_SEG55)*2,&buf_tx_232[700],(kol-ii)*2);
                                         // читаю по одному слову и упаковую в выходной массив
                                         // иначе происходит переполнение массива !!!! 4 10 2021
                                          for(k=0;k<(kol-ii);k++)
                                          {
                                            RdFromFleshToArr(STRUCT_US +(k + crc + ii-BEGIN_SEG55)*2,&flashka[0],2);
                                             *(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&flashka[0]);
                                            //*(unsigned int*)(mobus.point_out+(ii<<1)+(k<<1))=swap(*(unsigned int*)&buf_tx_232[700+(k<<1)]);
                                          }
                                          ii=kol;
                                          ...
                                          {  
                                         // RdFromFleshToArr(STRUCT_US +(crc+ii-BEGIN_SEG55)*2,&buf_tx_232[700],(END_SEG55-(crc+ii)+1)*2);  
                              ...
                              //  *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+Obj_ppp_tx.l_data]=crc_m1(&buf_tx_232[TR_OP_DATA+C1_DATA],Obj_ppp_tx.l_data,0xffff);
                                   ...
                                   *mobus.point_out++=mobus.modbus_crc_new;
                                   ...
                                   //    Obj_ppp_tx.l_data=3+kol*2; //без crc
                                       ...
                                       break;
                                 case 5:
                                      /////  *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+2]=swap(crc);//start
                                   ...
                                   // ожидаемое количество байт в выходной буфер
                                         ...
                                         *mobus.point_out++=*(mobus.point_in + mobus.index_elam);
                                         if (mobus.flag_elam==1) *mobus.point_out++=*(mobus.point_in+mobus.index_elam+mobus.smest_to_elam);
                                        *mobus.point_out++=*(mobus.point_in+mobus.index_elam+mobus.smest_to_elam+1);
                                        *mobus.point_out++=*(mobus.point_in+mobus.index_elam+mobus.smest_to_elam+2);
                                        *mobus.point_out++=*(mobus.point_in+mobus.index_elam+mobus.smest_to_elam+3);
                                        *mobus.point_out++=*(mobus.point_in+mobus.index_elam+mobus.smest_to_elam+4);
                                        *mobus.point_out++=*(mobus.point_in+mobus.index_elam+mobus.smest_to_elam+5);
                                   ...
                                   *mobus.point_out++=mobus.modbus_crc_new;
                                    mobus.count_out_elam+=Obj_ppp_tx.l_data + 2;   // плюс контрольная сумма
                                       ...
                                       if (sel_modul!=1) 
                                       {
                                        ...
                                        {
                                        case MB_TU1:
                                          if(c1_byte.mb[0]==0x55)  break; // не давать включать ту безконтрольно
                                          if(kol==0xff00) {TU1_ON;}
                                          if(kol==0x0000) {TU1_OFF;}
                                                   break;
                                        case MB_TU2:
                                                   if(kol==0xff00) {TU2_ON;}
                                                   if(kol==0x0000) {TU2_OFF;}
                                                   break;
                                        case MB_TU1_IMP:
                                          if(c1_byte.mb[0]==0x55)  break; // не давать включать ту безконтрольно
                                          ...
                                          if ( cnt_tu1 == 0 || cnt_tu1 == 0xff) cnt_tu1=TM_TU_IMP;
                                        ...
                                        case MB_TU2_IMP:
                                          ...
                                          if ( cnt_tu2 == 0  || cnt_tu2 == 0xff) cnt_tu2 = TM_TU_IMP;
                                        ...
                                        //////////////////////////////////
                                        case MB_BPA:
                                                   if(kol==0xff00) {bpa.sost=1;break;}
                                                   if(kol==0x0000) {bpa.sost=3;break;}
                                        ...
                                        {
                                          if(crc==MB_COM_DISP)com_disp_mbus();   
                                        }
                                 ...
                                 case 6:
                                       ...
                                       //////////07 11 2019 *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+2]=swap(crc);//start
                                       //////////07 11 2019 *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+4]=swap(kol);
                                   ...
                                   *mobus.point_out++=mobus.modbus_crc_new;
                                    mobus.count_out_elam+=Obj_ppp_tx.l_data + 2;   // плюс контрольная сумма
                                       ...
                                       *(unsigned int*)&buf_tx_232[1000]=kol;
                                        ...
                                        {
                                          WrArrayToFlesh(A_SEG9+(crc-BEGIN_SEGWR9)*2,&buf_tx_232[1000],2,0,0); 
                                       ...
                                       if((crc>=BEGIN_SEGWR3)&&(crc<=END_SEGWR3))
                                        {
                                          RdFromFleshToArr(A_SEG3+(crc-BEGIN_SEGWR3)*2,&buf_tx_232[1050],2);
                                          WrArrayToFlesh(A_SEG3+(crc-BEGIN_SEGWR3)*2,&buf_tx_232[1000],2,0,0); 
                                          RdFromFleshToArr(A_SEG3,(unsigned char *)&c2_byte.mb[0],SEG3*2);
                                          if(*(unsigned int*)&buf_tx_232[1050]!=*(unsigned int*)&buf_tx_232[1000])refresh_a(crc-BEGIN_SEG3);
                                         
                                        }
                                       
                                          if((crc>=BEGIN_SEGWR8)&&(crc<=END_SEGWR8))
                                        {
                                          WrArrayToFlesh(A_SEG8+(crc-BEGIN_SEGWR8)*2,&buf_tx_232[1000],2,0,0); 
                                   //       RdFromFleshToArr(A_SEG8,(unsigned char *)&c_config_tc[0],SEG8*2);
                                          if((crc-BEGIN_SEGWR8)==0x17 )   {k_a_tit[0]=0; k_a_tit[1]=0;}
                                        ...
                                        {
                                          WrArrayToFlesh(A_SEG99+(crc-BEGIN_SEGWR99)*2,&buf_tx_232[1000],2,0,0); 
                                   
                                          RdFromFleshToArr(A_SEG99,(unsigned char *)&c1_byte.mb[0],SEG99*2);
                                       ...
                                       if((crc>=BEGIN_SEGWR6)&&(crc<=END_SEGWR6))
                                        {
                                          WrArrayToFlesh(A_SEG6+(crc-BEGIN_SEGWR6)*2,&buf_tx_232[1000],2,0,0); 
                                        }
                                       ...
                                       if((crc>=BEGIN_SEGWR4)&&(crc<=END_SEGWR4))
                                        {
                                          ...
                                          if(crc==MB_SEC)if(kol<60){buf_tx_232[1000]=kol;flag_time=1;}
                                          if(crc==MB_MIN)if(kol<60){buf_tx_232[1001]=kol;flag_time=1;}
                                          if(crc==MB_HR) if(kol<24){buf_tx_232[1002]=kol;flag_time=1;}
                                          if(crc==MB_DAY)if(kol<32){buf_tx_232[1003]=kol;flag_time=1;}
                                          if(crc==MB_MONTH)if(kol<13)if(kol!=0){buf_tx_232[1004]=kol;flag_time=1;}
                                          if(crc==MB_YEAR)
                                            {
                                            // year=kol-2000;
                                            // if(year<100){buf_tx_232[1005]=year;flag_time=1;}
                                            if(kol<100){buf_tx_232[1005]=kol;flag_time=1;}
                                            }  
                                        if(flag_time!=0) 
                                        {
                                         old_unix=unix;
                                         ...
                                         burst_ds_r();
                                        // if ((( old_unix - unix) >= 600 )||(( unix - old_unix) >= 600 )) bit_level=1;        
                                        if(unix>old_unix)
                                         {
                                          if (( unix - old_unix) >= 600 ) bit_level=1;   
                                         }
                                         else
                                         {
                                           if ((old_unix-unix) >= 600 ) bit_level=1; 
                                         }

                                         if ( ( sost_psm==4)  && (bit_level==1) )bit_level_psm=1;
                                        }
                                   ...
                                   *mobus.point_out++=mobus.modbus_crc_new;
                                    mobus.count_out_elam+=Obj_ppp_tx.l_data + 2;   // плюс контрольная сумма
                                      ...
                                      RdFromFleshToArr(A_SEG3,&buf_tx_232[1050],16);
                                       ...
                                       /////07 11 2019 *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+2]=swap(crc);//start
                                       /////07 11 2019 *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+4]=swap(kol);//start
                                       /////////////////////////////////!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                                    /////////////////////////////////!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                                   ...
                                   for(ii=0;ii<kol;ii++)
                                    ...
                                    if(((crc+ii)>=BEGIN_SEGWR9)&&((crc+ii)<=END_SEGWR9))
                                    {
                                      if((crc+kol-1)<=END_SEGWR9)
                                          {
                                          for(k=0;k<(kol-ii);k++) *(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1))=swap(*(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1)));
                                          WrArrayToFlesh(A_SEG9+(crc+ii-BEGIN_SEGWR9)*2,(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)),(kol-ii)*2,0,0);                                      
                                          ii=kol;
                                          }
                                        
                                      else
                                          {  
                                          for(k=0;k<(END_SEG9-(crc+ii)+1);k++) *(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1))=swap(*(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1)));
                                          WrArrayToFlesh(A_SEG9+(crc+ii-BEGIN_SEGWR9)*2, (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)),(kol-ii)*2,0,0);
                                          ii=END_SEGWR9-crc;
                                          }
                             ...
                             //        RdFromFleshToArr(A_SEG9,(unsigned char *)&ust_vhod,SEG9*2); 
                                    ...
                                    if((crc+ii)==MB_TII1){modbus_mem1[AD_TII1]=swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)));cnt_tii[0]=modbus_mem1[AD_TII1];flag_tii=1;}
                                    if((crc+ii)==MB_TII2){modbus_mem1[AD_TII2]=swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)));cnt_tii[1]=modbus_mem1[AD_TII2];flag_tii=1;}
                                    if((crc+ii)==MB_TII3){modbus_mem1[AD_TII3]=swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)));cnt_tii[2]=modbus_mem1[AD_TII3];flag_tii=1;}
                                    if((crc+ii)==MB_TII4){modbus_mem1[AD_TII4]=swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)));cnt_tii[3]=modbus_mem1[AD_TII4];flag_tii=1;}
                                    if((crc+ii)==MB_TII5){modbus_mem1[AD_TII5]=swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)));cnt_tii[4]=modbus_mem1[AD_TII5];flag_tii=1;}
                                    if((crc+ii)==MB_TII6){modbus_mem1[AD_TII6]=swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)));cnt_tii[5]=modbus_mem1[AD_TII6];flag_tii=1;}
                                    if((crc+ii)==MB_TII7){modbus_mem1[AD_TII7]=swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)));cnt_tii[6]=modbus_mem1[AD_TII7];flag_tii=1;}
                                    if((crc+ii)==MB_TII8){modbus_mem1[AD_TII8]=swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)));cnt_tii[7]=modbus_mem1[AD_TII8];flag_tii=1;}
                 
                                    if(((crc+ii)>=(BEGIN_SEGWR+8))&&((crc+ii)<=END_SEGWR)) modbus_mem1[crc+ii-BEGIN_SEGWR]=swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)));
                                    ...
                                    if(((crc+ii)>=BEGIN_SEGWR3)&&((crc+ii)<=END_SEGWR3))
                                    {
                                      ...
                                      if((crc+kol-1)<=END_SEGWR3)
                                          {
                                          for(k=0;k<(kol-ii);k++) *(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1))=swap(*(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1)));
                                          WrArrayToFlesh(A_SEG3+(crc+ii-BEGIN_SEGWR3)*2, (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)),(kol-ii)*2,0,0);                                      
                                          ii=kol;
                                          }
                                        
                                      else
                                          {  
                                          for(k=0;k<(END_SEG3-(crc+ii)+1);k++) *(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1))=swap(*(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1)));
                                          WrArrayToFlesh(A_SEG3+(crc+ii-BEGIN_SEGWR3)*2, (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)),(kol-ii)*2,0,0);
                                          ii=END_SEGWR3-crc;
                                          }
                               ...
                               if(((crc+ii)>=BEGIN_SEGWR8)&&((crc+ii)<=END_SEGWR8))
                                    {
                                      if((crc+kol-1)<=END_SEGWR8)
                                          {
                                          for(k=0;k<(kol-ii);k++) *(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1))=swap(*(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1)));
                                          WrArrayToFlesh(A_SEG8+(crc+ii-BEGIN_SEGWR8)*2, (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)),(kol-ii)*2,0,0);                                      
                                            if ((crc+kol-1)==END_SEGWR8)   {k_a_tit[0]=0; k_a_tit[1]=0;}
                                          ii=kol;
                                          }
                                        
                                      else
                                          {  
                                          for(k=0;k<(END_SEG8-(crc+ii)+1);k++) *(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1))=swap(*(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1)));
                                          WrArrayToFlesh(A_SEG8+(crc+ii-BEGIN_SEGWR8)*2, (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)),(kol-ii)*2,0,0);
                                          ii=END_SEGWR8-crc;
                                          }
                                   ...
                                   /// ГЗУ
                                    if(((crc+ii)>=BEGIN_SEGWR99)&&((crc+ii)<=END_SEGWR99))
                                    {
                                      if((crc+kol-1)<=END_SEGWR99)
                                          {
                                          for(k=0;k<(kol-ii);k++) *(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1))=swap(*(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1)));
                                          WrArrayToFlesh(A_SEG99+(crc+ii-BEGIN_SEGWR99)*2, (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)),(kol-ii)*2,0,0);                                      
                                          ii=kol;
                                          }
                                        
                                      else
                                          {  
                                          for(k=0;k<(END_SEG99-(crc+ii)+1);k++) *(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1))=swap(*(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1)));
                                          WrArrayToFlesh(A_SEG99+(crc+ii-BEGIN_SEGWR99)*2, (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)),(kol-ii)*2,0,0);
                                          ii=END_SEGWR99-crc;
                                          }
                                    ...
                                    if(((crc+ii)>=BEGIN_SEGWR6)&&((crc+ii)<=END_SEGWR6))
                                    {
                                      ...
                                      if((crc+kol-1)<=END_SEGWR6)
                                          {
                                          for(k=0;k<(kol-ii);k++) *(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1))=swap(*(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1)));
                                          WrArrayToFlesh(A_SEG6+(crc+ii-BEGIN_SEGWR6)*2, (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)),(kol-ii)*2,0,0);                                      
                                          ii=kol;
                                          }
                                        
                                      else
                                          {  
                                          for(k=0;k<(END_SEG6-(crc+ii)+1);k++) *(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1))=swap(*(unsigned int*)(mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)+(k<<1)));
                                          WrArrayToFlesh(A_SEG6+(crc+ii-BEGIN_SEGWR6)*2, (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)),(kol-ii)*2,0,0);
                                          ii=END_SEGWR6-crc;
                                          }
                                    ...
                                    if((crc+ii)==MB_SEC)
                                           if(swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)))<60)
                                                 {buf_tx_232[1000]=buf_rx_ppp[offset+C1_DATA+16+(ii<<1)];flag_time=1;}
                                    if((crc+ii)==MB_MIN)
                                           if(swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)))<60)
                                                 {buf_tx_232[1001]=buf_rx_ppp[offset+C1_DATA+16+(ii<<1)];flag_time=1;}
                                   
                                    if((crc+ii)==MB_HR)
                                           if(swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)))<24)
                                                 {buf_tx_232[1002]=buf_rx_ppp[offset+C1_DATA+16+(ii<<1)];flag_time=1;}
                                    
                                    
                                    if((crc+ii)==MB_DAY)
                                           if(swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)))<32)
                                                 {buf_tx_232[1003]=buf_rx_ppp[offset+C1_DATA+16+(ii<<1)];flag_time=1;}
                                    
                                    if((crc+ii)==MB_MONTH)
                                           if(swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)))<13)
                                                 if(swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)))!=0)
                                                         {buf_tx_232[1004]=buf_rx_ppp[offset+C1_DATA+16+(ii<<1)];flag_time=1;}
                                    
                                    if((crc+ii)==MB_YEAR)
                                        {
                                          //year=swap(*(unsigned int*)&buf_rx_ppp[offset+C1_DATA+15+(ii<<1)])-2000;
                                          year=swap(*(unsigned int*) (mobus.point_in+mobus.index_elam + mobus.smest_to_elam + 7 +(ii<<1)));
                                          if(year<100){buf_tx_232[1005]=year;flag_time=1;}
                                        }
                                    ...
                                    if(flag_refresh==1)
                                    {
                                      ...
                                      for(k=0;k<8;k++)
                                      {
                                       if(*(unsigned int*)&buf_tx_232[900+k*2]!=*(unsigned int*)&buf_tx_232[1050+k*2])refresh_a(k);
                                   ...
                                   */
                                    ...
                                    if(unix>old_unix)
                                      ...
                                      if(flag_tii!=0)crc_tii=func_crc_tii();
                                     /////07 11 2019 Obj_ppp_tx.l_data=6; 
                               ...
                               case 0x9003: 
                                          ...
                                          break;
                                       */
                               default: return(0);  // выход по ошибке модбас пакета
                               }
                               
                  if (mobus.flag_elam==1) goto ReEnter;
                  
FinitaPerformence:                  
// switch обработчик модбас фрагмента                         закончен **************************************
                            
                               // этот кусок не нужен срс считается в каждом фрагменте
                              //  *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+Obj_ppp_tx.l_data]=crc_m1(&buf_tx_232[TR_OP_DATA+C1_DATA],Obj_ppp_tx.l_data,0xffff);       //crc                               
                          ...
                          //      Obj_ppp_tx.l_data=Obj_ppp_tx.l_data+4;   // длина данных плюс 2 байта срс плюс 2 байта 
...
{
  if ( (sel_modul==1) && (pnt_buf[2]>4)) return(0); // мкд
  if ( (sel_modul!=1) && (pnt_buf[2]>=8)) return(0); // мтс
...
{
if (pnt_buf[2]>1)return(0);
buf_tx_232[TR_OP_DATA+C1_DATA+2]=pnt_buf[2];
*(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+3]=modbus_mem1[AD_TIT1+pnt_buf[2]];
Obj_ppp_tx.l_data=6;
return(1);
}
...
buf_tx_232[TR_OP_DATA+C1_DATA+2]=pnt_buf[2];
*(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+3]=modbus_mem1[AD_TII1+pnt_buf[2]]; 
Obj_ppp_tx.l_data=6;
return(1);          
}
...
char i,k,r;
k=0;  
if(buf_tx_232[TR_OP_DATA+C1_DATA]==6)k=1; 
  if  (sel_modul==1) r=5;
        else r=8;
  
for(i=k;i<r;i++) modbus_mem1[AD_TII1+i]=0;
buf_tx_232[TR_OP_DATA+C1_DATA+2]=0;
Obj_ppp_tx.l_data=4;
}
...
{
  char i;
  for(i=0;i<16;i++) *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+2+(i<<1)]=modbus_mem1[AD_TII1+i];  
...
{
  char i; 
     for(i=0;i<8;i++) *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+2+(i<<1)]=modbus_mem1[AD_TIT1+i];     
...
buf_tx_232[TR_OP_DATA+C1_DATA+2]=pnt_buf[2];
    ...
    if(pred_sost==sost) buf_tx_232[TR_OP_DATA+C1_DATA]=192;
    Obj_ppp_tx.l_data=4; 
}
...
unsigned char proc_kamintel(unsigned char *pnt_buf,unsigned char length)
{
  if(pnt_buf[length-1] !=crc_485(length-1, &pnt_buf[0]))return(0);
  ...
  switch(pnt_buf[0])
   ...
   case 0: if (null_tii_485(&pnt_buf[0])==0)return(0);else break;
   case 1: if (extract_tit_485(&pnt_buf[0])==0)return(0);else break;
   case 3:if (extract_tii_485(&pnt_buf[0])==0)return(0);else break;
   case 4: tc_485();break;
   case 5: 
   case 6:  null_all_tii_485();break;
   case 10: extract_all_tii_485();break;
   case 11: extract_all_tit_485();break;
   case 20: obmen_full=0;return(0);
    
   case 128: com_disp(&pnt_buf[0]);break;//exo_485();break;
    
    
   case 201: w_in_out_485(&pnt_buf[0]);break;  // задержка на вход выход записать
   case 200: r_in_out_485();break;  // задержка на вход выход прочитать

    
    default:return(0);
    }
  
  buf_tx_232[TR_OP_DATA+C1_DATA+Obj_ppp_tx.l_data-1]=crc_485(Obj_ppp_tx.l_data-1,&buf_tx_232[TR_OP_DATA+C1_DATA]);
  return(1);
}
...
unsigned int proc_modbus(unsigned char *pnt_buf,unsigned char length)
 {
   ...
   unsigned char i;
     ...
     pnt_buf[length-1]=i;                            
     if(*(unsigned int*)&pnt_buf[length-2]!=crc_m1(&pnt_buf[0],length-2,0xffff))return(0); //crc
   
   
   switch (pnt_buf[1])
   {
   case 1:if(length!=8)return(0);
        if(swap(*(unsigned int*)&pnt_buf[4])==0 || swap(*(unsigned int*)&pnt_buf[4])>2000){temp=pnt_buf[1];temp=temp<<8;return(temp | 0x8003);}
        if(swap(*(unsigned int*)&pnt_buf[4])>8)return(0);
        return(1);
   case 3:
   case 4:
      if(length!=8)return(0);
      if(swap(*(unsigned int*)&pnt_buf[4])==0 || swap(*(unsigned int*)&pnt_buf[4])>125){temp=pnt_buf[1];temp=temp<<8;return(temp | 0x8003);}
      if (( 0xffff - swap(*(unsigned int*)&pnt_buf[2])) < ( swap(*(unsigned int*)&pnt_buf[4])-1 )){temp=pnt_buf[1];temp=temp<<8;return(temp | 0x8002);}
      break;   
   case 5:
      if(length!=8)return(0);
      if(swap(*(unsigned int*)&pnt_buf[4])!=0 && swap(*(unsigned int*)&pnt_buf[4])!=0xff00){temp=pnt_buf[1];temp=temp<<8;return(0x8503);}
      return(5);
   case 6:
     if(length!=8)return(0);
     return(6);  
   case 16:
     ...
     if(swap(*(unsigned int*)&pnt_buf[4])==0 || swap(*(unsigned int*)&pnt_buf[4])>120)return(0x9003);
     temp=pnt_buf[5];
     if(temp!=swap(*(unsigned int*)&pnt_buf[4]))return(0x9003);
     if ((0xffff -swap(*(unsigned int*)&pnt_buf[2])) < (swap( *(unsigned int*)&pnt_buf[4])-1 ))return(0x9002);
     return(16);
   default: return(0);     
   }
...
next_elam:
            ...
            switch (* ( mobus.point_in +  mobus.index_elam + 2))
                ...
                case 6:  mobus.size_of_pac=7;break;
                case 16:  mobus.size_of_pac=8 + *( mobus.point_in +  mobus.index_elam + 7);break;
                default : {respons();return;}
                }
                if ( mobus.count_in_elam < ( mobus.index_elam +  mobus.size_of_pac + 2) )  {respons();return;}
               
               
// проверка фрагмента на CRC
 mobus.modbus_crc_new=crc_m1(mobus.point_in + mobus.index_elam, size_of_pac , 0xffff);
             if(mobus.modbus_crc_new!=(  (  (unsigned int)( * (mobus.point_in + mobus.index_elam + mobus.size_of_pac) )<<8)
                             + (   unsigned int)( * (mobus.point_in + mobus.index_elam+size_of_pac + 1) ) )  )     {respons();return;}
