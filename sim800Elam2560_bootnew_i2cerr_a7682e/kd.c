...
char read_ohr(void) {
  ...
  if (point_head == point_tail) {
    ...
    if (modbus_mem1[AD_IND_DNS] == 0)
      ...
      bufer[1] = modbus_mem1[AD_IND_DNS] - 1;
...
void test_t_vhod(void) {
  ...
  if (timer_vhod != 0) {
    ...
    sost = SNAT;
    if (sel_modul == 1)
      PORTB = PORTB & ~0x04; //  PORTE=PORTE & ~4;  //ravil
    insert_query(nomer_tab, sost);
...
void zp_prihod_uhod(void) {
  ...
  case 1:
    insert_zapis_kd(nomer_tab, ZP_PRIHOD);
    flag_arhiv_prihod = 0;
    break;
  case 2:
    insert_zapis_kd(nomer_tab, ZP_UHOD);
    flag_arhiv_prihod = 0;
    break;
  default:
    break;
