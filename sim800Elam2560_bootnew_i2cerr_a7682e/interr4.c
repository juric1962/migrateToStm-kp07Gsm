...
struct struct_ts str_tc1, str_tc2, str_tc3, str_tc4, str_tc5, str_tc6, str_tc7,
    str_tc8, str_tc15, str_tc16;
/// 11111111111111111111111111111 функции
...
__interrupt void TIMER0_COMPA_interrupt(void) {
  ...
  if (struct_tcc1.fl_ch_tc == 1) {
    ...
    if (struct_tcc1.count_tc == 0) {
      ...
      //    modbus_mem1[AD_TS]=(modbus_mem1[AD_TS] & (~(0x01<<14))) |
      //    (struct_tcc1.real_tc <<14);       // 15 02 2016
    }
  } else {
  ...
  if (struct_tcc2.fl_ch_tc == 1) {
    ...
    if (struct_tcc2.count_tc == 0) {
      ...
      //    modbus_mem1[AD_TS]=(modbus_mem1[AD_TS] & (~(0x01<<15))) |
      //    (struct_tcc2.real_tc <<15);  // 15 02 2016
    }
  } else {
...
next_step1:
  ...
  // if(Rs485_1.cnt_tm_pre_tx==0) UCSR3B=UCSR3B | UDRIE;  // enable transmit
  // 485_1
  if (Rs485_1.cnt_tm_pre_tx == 0)
    send_485_1(); // enable transmit 485_1
...
lable_tm21:
  ...
  // if(Rs485_2.cnt_tm_pre_tx==0) UCSR1B=UCSR1B | UDRIE;  // enable transmit
  // 485_1
  if (Rs485_2.cnt_tm_pre_tx == 0)
    send_485_2(); // enable transmit 485_2
...
lable_tm22:
  ...
  //  if(Rs232_2.cnt_tm_pre_tx==0) UCSR2B=UCSR2B | UDRIE;  // enable transmit
  //  232_1
  if (Rs232_2.cnt_tm_pre_tx == 0)
    send_232_2(); // enable transmit 232_1
nt_step2:
...
lable_tm26:
  ...
  if (tim_prihod_uhod == 0) {
    ...
    if (nomer_tab != 254)
      bit_registr1 =
          bit_registr1 |
          TABLETKA; // после приветствия только запускать охранный процесс
                    // timer_samovzatia=0; //????
    // flag_sam_sam=0; //????
    // if( sost==4) dabl=0;
    //  TIFR=TIFR | 0x20;//????
...
erty79:
  ...
  glutch5--;
  if (glutch5 == 0)
    prihod_uhod = 4; //  режим ожидания двойного нажатия
...
next_tm1:
  ...
  if (fl_ip.ip_tm_cntr_en == 1) {
    ...
    if (ip_change == 1) {
      ip_change = 0;
      Control.cnt_link = 0;
    } // если поменялся IP тут же послать контроль канала !!!
    if (Control.cnt_link == 0) {
