...
struct struct_ts str_tc1,str_tc2,str_tc3,str_tc4,str_tc5,str_tc6,str_tc7,str_tc8,str_tc15,str_tc16;
///11111111111111111111111111111 функции
...
//void opros_ts(unsigned char num, unsigned char port, unsigned char pin, struct struct_ts *ts, unsigned int *tii)
void opros_ts(unsigned char num, unsigned char port, unsigned char pin, struct struct_ts *ts)

{
  if(port & pin)ts->tek_pin=0;else ts->tek_pin=1;
   if(ts->fl_ch_tc==1)
     {
       ts->count_tc--;
       if(ts->count_tc==0)
       ...
       //  modbus_mem1[num]=*tii;
         ...
         arr_tii_32[num]++;
         modbus_mem1[AD_TS]=(modbus_mem1[AD_TS] & (~(0x01<<num))) | (ts->tek_pin <<num);
         if(num==2){if((ts->tek_pin)==0)trevoga=1;else trevoga=0;}
       }
     } 
     else
     {
...
{   
   ...
   UDR0=buf_tx_232[count_tx_ppp]; if(Regim==RG_DEBAG) UDR3=buf_tx_232[count_tx_ppp];
   count_tx_ppp++;
}
...
if (fl_reg3.add_byte==1) 
    {         
      fl_reg3.add_byte=0;
      if (buf_tx_232[count_tx_ppp]==0x7e){UDR0=0x5e;if(Regim==RG_DEBAG) UDR3=0x5e;return;}  
      if (buf_tx_232[count_tx_ppp]==0x7d){UDR0=0x5d;if(Regim==RG_DEBAG) UDR3=0x5d;return;} 
      UDR0=buf_tx_232[count_tx_ppp]+0x20;if(Regim==RG_DEBAG) UDR3=buf_tx_232[count_tx_ppp]+0x20;
      return;
    }
...
exit_tx:  UDR0=buf_tx_232[count_tx_ppp]; if(Regim==RG_DEBAG) UDR3=buf_tx_232[count_tx_ppp];
} 
...
#pragma vector=TIMER0_COMPA_vect       
     __interrupt void TIMER0_COMPA_interrupt(void)
 {
   ...
   if(PING & TCC1)struct_tcc1.tek_pin=1;else struct_tcc1.tek_pin=0;
   ...
   if(struct_tcc1.fl_ch_tc==1)
     ...
     //    modbus_mem1[AD_TS]=(modbus_mem1[AD_TS] & (~(0x01<<14))) | (struct_tcc1.real_tc <<14);       // 15 02 2016
       }
     } 
     else
     {
   ...
   if(PINA & TCC2)struct_tcc2.tek_pin=1;else struct_tcc2.tek_pin=0;
   ...
   if(struct_tcc2.fl_ch_tc==1)
     ...
     //    modbus_mem1[AD_TS]=(modbus_mem1[AD_TS] & (~(0x01<<15))) | (struct_tcc2.real_tc <<15);  // 15 02 2016
       }
     } 
     else
     {
  ...
  if (cnt_clr_wdt>=250)
    ...
    if(fl_cts_232.on==1)
    {
      if(fl_cts_232_ignor==TRUE)
            {
             fl_cts_232.on=0;
             if(count_tx_ppp==0){S2_RD;UDR0=buf_tx_232[0];if(Regim==RG_DEBAG) UDR3=buf_tx_232[0];}
...
next_step1:
  ...
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
    ...
    if(Rs485_1.cnt_tm_pre_tx==0) goto  nxt_step2;
    Rs485_1.cnt_tm_pre_tx--;
    //if(Rs485_1.cnt_tm_pre_tx==0) UCSR3B=UCSR3B | UDRIE;  // enable transmit 485_1 
    if(Rs485_1.cnt_tm_pre_tx==0) send_485_1();  // enable transmit 485_1
    
  nxt_step2:
                       
                if(Rs485_1.cnt_tm_out==0) goto nxt_step3;
                Rs485_1.cnt_tm_out--;
 ...
 nxt_step3:      
                 ...
                 if(Rs485_1.cnt_tm_rx_out==0) goto nxt_step4;
 ...
 nxt_step4:                  
                if(Rs485_1.cnt_tm_tx_out==0) goto lable_tm21;
                Rs485_1.cnt_tm_tx_out--;
                if(Rs485_1.cnt_tm_tx_out==1) {CLR_RTS1;} /*togle to receiv mode*/ 
                if(Rs485_1.cnt_tm_tx_out==0){CLR_RTS1;S3_OFF;Rs485_1.cnt_tm_out=Rs485_1.vol_tm_out;UCSR3B =UCSR3B | RXEN | RXCIE;} // togle to receiv mode ;
 ...
 //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
    ...
    if(Rs485_2.cnt_tm_pre_tx==0) goto  next_step2;
    Rs485_2.cnt_tm_pre_tx--;
   // if(Rs485_2.cnt_tm_pre_tx==0) UCSR1B=UCSR1B | UDRIE;  // enable transmit 485_1 
     if(Rs485_2.cnt_tm_pre_tx==0) send_485_2();  // enable transmit 485_2

    
  next_step2:
                       
                if(Rs485_2.cnt_tm_out==0) goto next_step3;
                Rs485_2.cnt_tm_out--;
 ...
 next_step3:      
                 ...
                 if(Rs485_2.cnt_tm_rx_out==0) goto next_step4;
 ...
 next_step4:                  
                if(Rs485_2.cnt_tm_tx_out==0) goto lable_tm22;
                Rs485_2.cnt_tm_tx_out--;
                if(Rs485_2.cnt_tm_tx_out==1) {CLR_RTS3; } /*togle to receiv mode*/ 
                if(Rs485_2.cnt_tm_tx_out==0){CLR_RTS3;S4_OFF;Rs485_2.cnt_tm_out=Rs485_2.vol_tm_out;UCSR1B =UCSR1B | RXEN | RXCIE;} // togle to receiv mode ;
 ...
 //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
    ...
    if(Rs232_2.cnt_tm_pre_tx==0) goto  nt_step2;
    Rs232_2.cnt_tm_pre_tx--;
  //  if(Rs232_2.cnt_tm_pre_tx==0) UCSR2B=UCSR2B | UDRIE;  // enable transmit 232_1 
    if(Rs232_2.cnt_tm_pre_tx==0) send_232_2();  // enable transmit 232_1 
  nt_step2:
                       
                if(Rs232_2.cnt_tm_out==0) goto nt_step3;
                Rs232_2.cnt_tm_out--;
 ...
 nt_step3:      
                 ...
                 if(Rs232_2.cnt_tm_rx_out==0) goto nt_step4;
 ...
 nt_step4:                  
                if(Rs232_2.cnt_tm_tx_out==0) goto lable_tm23;
                Rs232_2.cnt_tm_tx_out--;
                if(Rs232_2.cnt_tm_tx_out==1) {CLR_RTS2;} /*togle to receiv mode*/ 
                if(Rs232_2.cnt_tm_tx_out==0){CLR_RTS2;S5_OFF;Rs232_2.cnt_tm_out=Rs232_2.vol_tm_out;UCSR2B =UCSR2B | RXEN | RXCIE;} // togle to receiv mode ;
...
//if (timer_vhod!=0)timer_vhod--;
//if (timer_autovzat!=0)timer_autovzat--;


             if(tim_prihod_uhod==0) goto erty78;
             tim_prihod_uhod--;
             if(tim_prihod_uhod==0) 
                 {
                  prihod_uhod=0;
                 if(nomer_tab !=254) bit_registr1=bit_registr1 | TABLETKA;   // после приветствия только запускать охранный процесс
                // timer_samovzatia=0; //????
                 //flag_sam_sam=0; //????
                // if( sost==4) dabl=0;
               //  TIFR=TIFR | 0x20;//????
...
erty78: 
             if(window_prihod_uhod==0) goto erty79;
             window_prihod_uhod--;
           //  if (window_prihod_uhod < 6000) PORTB=PORTB ^ 0x08;
            if(window_prihod_uhod==0) 
               {
                tim_prihod_uhod=3400;
                //TIMSK=TIMSK &~0x20;    //????// запретить ключ на это время
                EIMSK=EIMSK & ~ 0x02;    // disable myself
                if (podnos==1) {prihod_uhod=1;dabl=1;}    // это приход
                if (podnos>1) {prihod_uhod=2; dabl=2;}    // это уход
               podnos=0;
               }
...
erty79: 
   ...
   if (glutch5 == 0 ) goto t_345;
                             glutch5--;
                    if(glutch5==0) prihod_uhod=4;               //  режим ожидания двойного нажатия
                    
             t_345: ;      
    
 glutch++;  
 if (glutch >4000) glutch=0;
    
if (glutch6!=0)
 {
   glutch6--;
   if(glutch6==0) bit_registr1=bit_registr1 & ~BLOCK_TABLETKA; 
 }
...
if (timer_autovzat!=0)timer_autovzat--;
      ...
      {
        cnt_tu1--;
        if(cnt_tu1==0) {TU1_OFF;}
      }
         
      if(cnt_tu2!=0)
      {
        cnt_tu2--;
        if(cnt_tu2==0) {TU2_OFF;}
      }
   ...
   */ 
    ...
    //if(count_block_net!=0) count_block_net--;
      ...
      if(struct_tcc1.cnt_block_tc!=0) struct_tcc1.cnt_block_tc--;     
      if(struct_tcc2.cnt_block_tc!=0) struct_tcc2.cnt_block_tc--; 
...
next_tm001:   
    ...
    if(fl_rewrite.cnt_reset==0)
        ...
        *(unsigned int*)&buf_tx_232[L_TRAF-2]=crc_m1(&buf_tx_232[0],L_TRAF-2,0xffff);
...
next_tm0:   
 ...
 next_tm2:  
   ...
   //!!!!!!!!!!!!!dobavka
       if(cnt_no_link==0) goto next_tm3;
         cnt_no_link--;
         if(cnt_no_link==0)Fl_ch_sim=1;
  next_tm3:     
       
        if(cnt_link_res==0) goto next_tm4;
        cnt_link_res--;
        if(cnt_link_res==0)Fl_ch_sim=1;
    next_tm4:        
...
#pragma vector=USART0_RX_vect
           ...
           __interrupt  void USART0_RX_interrupt(void)
                ...
                unsigned char data;                
                data=UDR0;
                if((Regim==MODEM_ONLY)||(Regim==MODEM_ONLY_R)){UDR2=data;return;} //dobavka             
                if(Regim==RG_DEBAG)UDR1=data; 
...
#pragma vector=USART2_RX_vect
           ...
           __interrupt  void USART2_RX_interrupt(void)
                ...
                if((Regim==MODEM_ONLY)||(Regim==MODEM_ONLY_R)){UDR0=data;return;}//dobavka 
                 if (fl_at_mom_232==1)
                {
...

{
 ...
 if(Rs232_2.cnt_bt_rx_tx==0) goto end_tx;
...
// 1/11059200 *13*128*256*3=115 msec
   ...
   if(count_summa_adc[i]==0)
      {
        if(i==0) {modbus_mem1[AD_TEMP]=summa_adc[0]>>8;temperatura=modbus_mem1[AD_TEMP];}
        else
            if(i==1) modbus_mem1[AD_TIT1]=summa_adc[1]>>8; 
                     else modbus_mem1[AD_TIT1+1]=summa_adc[3]>>8;
...
#pragma vector=PCINT2_vect
 ...
 {   
   ...
   if(sel_modul!=1)
   {
   if(PINK & IO1) modbus_mem1[AD_TS]=modbus_mem1[AD_TS] & (~0x01);  
    else 
    {
