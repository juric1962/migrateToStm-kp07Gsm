...
// extern void send_info(char size,char __flash *p,unsigned char fl_id,unsigned
// char id);
...
char err_psm, err_switch, cnt_try_ys, tekpsm, pred_mask, fin_ys, count_try,
    count_sboj, try_ty;
...
void insert_zapis_kd(unsigned char nomer, unsigned char type) {
  ...
  *(unsigned int *)&zapis[12] = crc_sum(&zapis[0], 12);
  ...
  WrArrayToFlesh(STRUCT_TII_HEAP + (int)modbus_mem1[AD_IND_DNS] * 14, &zapis[0],
                 14, 0, 0);
  modbus_mem1[AD_IND_DNS]++;
  if (modbus_mem1[AD_IND_DNS] > 99) {
...
void day_zamer(void) {
  ...
  zamer = c2_byte.mb[0 + (2 * day_chan)] +
          (c2_byte.mb[1 + (2 * day_chan)] & 7) * 256;
  ...
  */
  ...
  if ((*(int *)&zapis[12]) ==
      crc_sum(&zapis[0],
              12)) //!!!!!ДОЛЖНА БЫТЬ ЧТЕНИЕ!!!!!!!!! ПРОВЕРКРЙ СРС!!! записи
  {

    if (zapis[10] != day_chan)
      goto hlop; // ищем записи только для даного канала в куче

    tik.st[0] = zapis[0];
    tik.st[1] = zapis[1];
    tik.st[2] = zapis[2];
    tik.st[3] =
        zapis[3]; // круг назад и еще 200 секунд  чтобы не зацепить замеры
    if ((tik.unix >= (day_unix - 86400 - 200)) &&
        (tik.unix < (day_unix - 10))) {
      if ((zapis[5] == 5) && (zapis[4] == 0xa0)) {
        goto hlop;
      } // если есть суточная запись не писать
      day_good++;
...
hlop:
  if (day_zap > 99) {
    ...
    if (day_good == 0)
      goto no_zap; // если нет ни одного замера то нет и суточного
    ...
    *(int *)&zapis[12] = crc_sum(&zapis[0], 12);
    WrArrayToFlesh(STRUCT_TII_HEAP + (int)modbus_mem1[AD_IND_DNS] * 14,
                   &zapis[0], 14, 0, 0);

    /*
                arr_cnt_tii_heap++;
                if ( arr_cnt_tii_heap >99)
      {arr_cnt_tii_heap=0;arr_cnt_tii_heap_modbus=99;} else
      arr_cnt_tii_heap_modbus=arr_cnt_tii_heap-1;
...
unsigned long int set_time_day_zamer(char date, char month, char year) {
  ...
  //  if ( (real_time.r_date > 31) | (real_time.r_date <1) ) real_time.r_date =
  //  1; if (real_time.r_hor > 60) real_time.r_hor = 0; if (real_time.r_min >
  //  60) real_time.r_min = 0; if (real_time.r_sec > 60) real_time.r_sec = 1;
  //
  sek_32 =
...
void end_zamer(char m) {
  ...
  case 10: {
    ...
    timer_tii[m] =
        set_end_time((real_time.r_min / 10) * 10, real_time.r_hor,
                     real_time.r_date, real_time.r_month, real_time.r_year) +
        600;
  ...
  case 20: {
    timer_tii[m] =
        set_end_time((real_time.r_min / 20) * 20, real_time.r_hor,
                     real_time.r_date, real_time.r_month, real_time.r_year) +
        1200;
  ...
  case 30: {
    timer_tii[m] =
        set_end_time((real_time.r_min / 30) * 30, real_time.r_hor,
                     real_time.r_date, real_time.r_month, real_time.r_year) +
        1800;
  ...
  case 60: {
    timer_tii[m] = set_end_time(0, real_time.r_hor, real_time.r_date,
                                real_time.r_month, real_time.r_year) +
                   3600;
  ...
  case 120: {
    timer_tii[m] = set_end_time(0, (real_time.r_hor / 2) * 2, real_time.r_date,
                                real_time.r_month, real_time.r_year) +
                   7200;
  ...
  case 240: {
    timer_tii[m] = set_end_time(0, (real_time.r_hor / 4) * 4, real_time.r_date,
                                real_time.r_month, real_time.r_year) +
                   14400;
  ...
  case 360: {
    timer_tii[m] = set_end_time(0, (real_time.r_hor / 6) * 6, real_time.r_date,
                                real_time.r_month, real_time.r_year) +
                   21600;
  ...
  case 1440: {
    u_i = set_end_time(c2_byte.mb[34], c2_byte.mb[32], real_time.r_date,
                       real_time.r_month, real_time.r_year) +
          (unsigned long int)c2_byte.mb[36];
    if (u_i <= unix)
      timer_tii[m] = u_i + 86400; // если время окончания замера меньше текущего
    else
      timer_tii[m] = u_i + 5;
    lock_bit = lock_bit & ~(1 << m);
  ...
  default: {
    ...
    timer_tii[m] = unix + ((long int)c2_byte.mb[0 + 2 * m] +
                           (long int)(c2_byte.mb[1 + 2 * m] & 7) * 256) *
                              60;
    lock_bit = lock_bit & ~(1 << m);
...
void end_zamer_tit(char m) {
  ...
  case 5: {
    ...
    timer_tit[m] =
        set_end_time((real_time.r_min / 5) * 5, real_time.r_hor,
                     real_time.r_date, real_time.r_month, real_time.r_year) +
        300;
  ...
  case 60: {
    timer_tit[m] = set_end_time(0, real_time.r_hor, real_time.r_date,
                                real_time.r_month, real_time.r_year) +
                   3600;
  ...
  case 120: {
    timer_tit[m] = set_end_time(0, (real_time.r_hor / 2) * 2, real_time.r_date,
                                real_time.r_month, real_time.r_year) +
                   7200;
  ...
  case 240: {
    timer_tit[m] = set_end_time(0, (real_time.r_hor / 4) * 4, real_time.r_date,
                                real_time.r_month, real_time.r_year) +
                   14400;
  ...
  case 360: {
    timer_tit[m] = set_end_time(0, (real_time.r_hor / 6) * 6, real_time.r_date,
                                real_time.r_month, real_time.r_year) +
                   21600;
  ...
  case 1440: {
    u_i = set_end_time(c2_byte.mb[34], c2_byte.mb[32], real_time.r_date,
          ...
          (unsigned long int)c2_byte.mb[36];
    if (u_i <= unix)
      timer_tit[m] = u_i + 86400; // если время окончания замера меньше текущего
    else
      timer_tit[m] = u_i + 5;
  ...
  default: {
    ...
    timer_tit[m] = unix + ((long int)c2_byte.mb[0x40 + (2 * m)] +
                           (long int)(c2_byte.mb[0x41 + (2 * m)] & 7) * 256) *
                              60;
    lock_bit_tit = lock_bit_tit & ~(1 << m);
...
unsigned long int read_arhiv_tii(char m) //
{
  ...
  index = 0 + m * 2;
  t_i = ((unsigned long int)c2_byte.mb[index] +
         (unsigned long int)(c2_byte.mb[index + 1] & 7) * 256);
  if (t_i > 1440)
    return (0);
  if (t_i == 0)
    return (0);
...
void store_tii_lock(char m) // записать в архив значения 1 минутные
{
...

  {
    ...
    if (tii_moto.bit.type_zam == 1) {
      *(unsigned long int *)&zapis[6] = cnt_moto[m];
    }
...
char test_switch_psm(void) {
  if (c1_byte.mb[1 * 2] == 8) {
    if (pred_mask == 8) {
      if (((modbus_mem1[AD_TS] >> 4) & 0xf) != 1)
        return (0);
      return (1);
    }
    if ((pred_mask + 1) != ((modbus_mem1[AD_TS] >> 4) & 0xf))
      return (0);
    return (1);
  }
  ////////////////
...
void store_ys_minut(void) // записать в архив значения за минуту
{
  ...
  t_i = (((unsigned long int)c1_byte.mb[index] +
          (unsigned long int)c1_byte.mb[index + 1] *
              256)); // реальное время замера
  zapis[4] = t_i;
  zapis[5] = t_i >> 8;
...
void store_ys_lock(void) // записать в архив значения
{
  ...
  t_i = (((unsigned long int)c1_byte.mb[index] +
          (unsigned long int)c1_byte.mb[index + 1] * 256));
...
void local_psm(void) {
  ...
  case 3: // ожидание коррекции
  {
    if (timer_psm == 0) {
      ...
      ////////////////////////
      RdFromFleshToArr(STRUCT_US + modbus_mem1[AD_IND_GZU] * 12,
                       (unsigned char *)&zapis[0], 12);
...

      {
        ...
        index = 6 * 2 + (tekpsm - 1) * 2;
        t_i = c1_byte.mb[index] + c1_byte.mb[index + 1] * 256;
        if ((tik.unix - unix) > ((unsigned long int)t_i * 60))
          goto new_zamer; //..
      ...
      new_zamer:
        ...
        unix_psm =
            unix + ((unsigned long int)t_i * 60); // время конец замера   //..
        bit_zamer_psm = 0;
        delta_ys = 0;
      }
      ////////////////////////
    }
    if (((modbus_mem1[AD_TS] >> 4) & 0xf) != tekpsm)
      sost_psm = 1;
    break;
  }
  ...
  for (i = 0; i < 16; i++) {
    ...
    */
    ...
    // if (ret_cod==0)
    {
      ...
      if ((matrix == tik.unix) && (zapiska[10] == m))
        return (1); // есть такая запись !
    }
...
// попытаться сформировать архивы, которые не сформировались по причине
// пропадания питания, по последним минутным замерам
//////////////////
// прочитать сначала архивы на предмет действительности
char try_form_arhiv(char m) //
{
...
// system_zap=22;
  ...
  // if (ret_cod==0)
  {
    ...
    {
      if (try_find_arhiv(tik.unix, m) == 1)
        return (1); // пытаюсь найти в стеке архивов запись с этим временем
                    // не нашли архив,
...
void start_a(void) // YES!!!!
{
  ...
  for (m = 14; m < 16; m++) {
    end_zamer(m);
    start_ttii[m] =
        arr_tii_32[m] - read_arhiv_tii(m); // начать новый минутный заме
  ...
  lock_bit = 0;
  time_sutok =
      set_time_day_zamer(real_time.r_date, real_time.r_month, real_time.r_year);
  if (time_sutok <= unix)
    time_sutok += 86400;
  for (m = 0; m < 2; m++)
    end_zamer_tit(m);
...
void start_aa(void) {
  ...
  if (time_sutok <= unix)
    time_sutok += 86400;
  // при смене времени все времена окончания замеров установить но начальное
  // значение тии не устанавливат
  if (sel_modul == 1) {
    for (m = 0; m < 5; m++)
      end_zamer(m);
  } else {
...
burst_ds_r();
  ...
  for (k=0;k<8;k++)
                        ...
                        if (unix >=timer_tii[k] )      // если ожидаемое время
больше текущего
                                                  {
                                                   tii_bit=tii_bit | (1<<k);
...
void energy_plus(void) {
  ...
  if (timer_psm != 0)
    timer_psm--; // время до переключения отсчитывается в секундах !!! // для
                 // ГЗУ
  ...
  for (k = 0; k < max; k++) {
    ...
    if ((tii_moto.bit.type_zam == 1) && ((modbus_mem1[AD_TS] >> k) & 1))
      cnt_moto[k]++;
    /*
    {
      if ( old_realsec[k] <= real_time.r_sec)
cnt_moto[k]+=real_time.r_sec-old_realsec[k]; else  cnt_moto[k]+=60 +
real_time.r_sec-old_realsec[k];
    }
...
*/
  ...
  for (k = 14; k < 16; k++) {
    ...
    if (  (tii_moto.bit.enable==1)  &&
          (((modbus_mem1[AD_TS] >> tii_moto.bit.nomer_tc) & 1
   )==tii_moto.bit.level_tc ) )
...

  {
    ...
    if (*(unsigned int *)&c2_byte.mb[46] == 1)
      day_zamer();
  }
