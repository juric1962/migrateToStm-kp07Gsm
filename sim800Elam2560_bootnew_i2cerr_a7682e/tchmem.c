                  ...
                  for(point_pos=0;point_pos<(4*10);point_pos=point_pos+4)
                      {
                             ...
                             for(i=0;i<4;i++)
                              ...
                              if(tab_proxy[(point_pos+i)] !=arr_ds[i+1]) goto next_tabl;
                  ...
                  for(point_pos=0;point_pos<(4*253);point_pos=point_pos+4)
                      {
                           ...
                           for(i=0;i<4;i++)
                              {
                              // сравниваем поднесенной таблеткой
                              if(buf[i] !=arr_ds[i+1]) goto next_tab2;
                              }
...
//
//
                if(PINK & TCH_I) data_ds=data_ds | 0x80;     //ravil
                else data_ds=data_ds & ~0x80;
...
//
    ...
    for (we=0;we<150;we++)
      {
                  delay(19);
// находим хот€бы два нулевых отсчета пор€д  
                  switch(sbr.sost)
                  {
                  case 0: {
                    if (PINK & TCH_I) break;
                  ...
                  case 1: {
                    if ((PINK & TCH_I)==0) { sbr.count_0++;break;}
                    else {  if(sbr.count_0 < 3) sbr.sost=2;
                            else {sbr.sost=3;sbr.count_0=0;}
                         }
                    break;
                    }
                   case 3: {
                    if (PINK & TCH_I) { sbr.count_0++; if(sbr.count_0 > 10) sbr.sost=4; break;}
                    else  sbr.sost=2;
                  ...
                  if( sbr.sost==2)  {delay(1871);return(0);}  // сбой при инициализации 
                  if( sbr.sost==4)  {delay(1871);return(1);}
      }
...
//              read pin
//
                for (cc=0;cc<8;cc++) arr_ds[cc]=read_pin();
 ...
 if(bit_registr1 & DS_DETECT)
   ...
   //            SOUND_ON;    !!!!!!!!!!!!!!!!!
                 ...
                 bit_registr1=bit_registr1 & ~DS_DETECT;
                    ...
                    if(bit_registr1 & BLOCK_TABLETKA) goto end_ds;
...
if ( (arr_ds[4]==0xff) || ( ak==1785) || ( ak==0 ) || ( tosch.kod > 1000000000 )) continue;   // по просьбе “ощевикова
            ...
            if((ds_crc==arr_ds[7]) )
                                    { 
                                      ...
                                      glutch6=1000; // автодекрементный таймер дл€ разблокировки считывани€ таблетки
                                  ...
                                  //    goto end_ds;
                                    ...
                                    // проверка повторного поднесени€ ключа
                                      
                                      if (podnos !=0) 
                                              {
                                                if(kodec[0]!=arr_ds[1]) { podnos=0;goto bad_pod;}
                                                if(kodec[1]!=arr_ds[2]) { podnos=0;goto bad_pod;}
                                                if(kodec[2]!=arr_ds[3]) { podnos=0;goto bad_pod;}
                                                if(kodec[3]!=arr_ds[4]) { podnos=0;goto bad_pod;}
                                                podnos++;
