...
void main(void)
 ...
 // unsigned char buf[11];
  ...
  //s_port(dbbb++);  // dbg
  load_sel_modul();
  if(sel_modul==1)init_pins_mkd_mod();
  if(sel_modul==0) init_pins_ts_mod();
  bufalo=1;
  if(sel_modul>1)  {sel_modul=1; WrArrayToFlesh(A_SEL_MODUL, &bufalo,1,0,0);init_pins_mkd_mod();}
  //s_port(dbbb++);  // dbg
  __watchdog_reset();
  ...
  if(check_keys()==1)
      { 
       while(1){__watchdog_reset();S1_YL;S2_YL;S3_YL;S4_YL;S5_YL;}
      }
  //s_port(dbbb++);  // dbg
...
//  if((Regim==MODEM_ONLY)||(Regim==MODEM_ONLY_R)){init_modem_only();} //dobavka 01 09 25
  
  
  
//  if(Regim==MODEM_CALL) {init_modem_call();}     25 04 2013
  
  if(Regim==RG_DEBAG){init_ports_debug();}
