...
__flash char stm_pwr_kon[]=       {'S','T','M',':','p','w','r',' ','k','o','n'};
__flash char stm_pwr_off[]=       {'S','T','M',':','p','w','r',' ','o','f','f',};
__flash char stm_pwr_koff[]=      {'S','T','M',':','p','w','r',' ','k','o','f','f',};
...
__flash char stm_base_sim[]=      {'S','T','M',':','b','a','s','e',' ','s','i','m'}; //dobavka
__flash char stm_res_sim[]=       {'S','T','M',':','r','e','s',' ','s','i','m'}; //dobavka
...
__flash char evm_ppp_err[]=   {'E','V','M',':','p','p','p',' ','e','r','r'};
__flash char evm_dcd_err[]=   {'E','V','M',':','d','c','d',' ','e','r','r'};
__flash char evm_no_rec[]=     {'E','V','M',':','n','o',' ','r','e','c'};
__flash char evm_cts_err[]=   {'E','V','M',':','c','t','s',' ','e','r','r'}; //dobavka
__flash char evm_ch_sim[]=    {'E','V','M',':','c','h',' ','s','i','m'};     //dobavka

//__flash char evc_heat_on[]=        {'E','V','C',':','h','e','a','t',' ','o','n'};
//__flash char evc_heat_off[]=       {'E','V','C',':','h','e','a','t',' ','o','f','f'};
__flash char evc_taken[]=          {'E','V','C',':','t','a','k','e','n'};
__flash char evc_alarm[]=          {'E','V','C',':','a','l','a','r','m'};
//__flash char evc_feed_akb[]=       {'E','V','C',':','f','e','e','d',' ','a','k','b'};
//__flash char evc_feed_220[]=       {'E','V','C',':','f','e','e','d',' ','2','2','0'};
...
__flash char msg_evc[]=          {'M','S','G',':','e','v','c',};
__flash char msg_ctrl_ch[]=      {'M','S','G',':','c','t','r','l',' ','c','h'};
__flash char msg_ctrl_nat[]=     {'M','S','G',':','c','t','r','l',' ','n','a','t'};


__flash char req_out_ctrl_cl[]={'R','E','Q','-','>','c','t','r','l',' ','c','l'};
...
struct                   //структура, описывающая объект передачи по PPP 
 ...
 unsigned long int vol_cnt_no_link, vol_cnt_link_res, cnt_no_link, cnt_link_res;; //dobavka
 ...
 if(buf[0]!=0)
   {
    ...
    kol=0;
       ...
       temp=strlen(&buf[j]);
       if(temp>35){e_rw_pdp_r=0xff;return;}
       buf_tx_232[j]=temp;
       for(i=0;i<temp;i++) {buf_tx_232[j+1]=buf[j];j++;}
       kol=kol+temp;
       ...
       j++;
       temp=strlen(&buf[j]);
       if(temp>15){e_rw_pdp_r=0xff;return;}
       buf_tx_232[j]=temp;
       for(i=0;i<temp;i++) {buf_tx_232[j+1]=buf[j];j++;}
       kol=kol+temp;
       ...
       kol=kol+3;      
       if(kol>(L_CR_GPRS_MAX-2)){e_rw_pdp_r=0xff;return;}
       
       *(unsigned int*)&buf_tx_232[L_C_GPRS_MAX-2]=crc_m1(&buf_tx_232[0],L_CR_GPRS_MAX-2,0xffff);
...
if(*(unsigned int*)&buf[L_TRAF-2]!=crc_m1(&buf[0],L_TRAF-2,0xffff))
  {      
...
for(m=0;m<8;m++)
 ...
 if(*(int*)&buf[12]!=crc_sum(&buf[0],12))WrArrayToFlesh(STRUCT_TII+m*14,0,14,1,0); 
 }
...
if ( arr_cnt_tii_heap >99  | arr_cnt_tii_heap ==0 ) {WrArrayToFlesh(A_IND_DNS,0,2,1,0);arr_cnt_tii_heap=0;}
*/                                      
modbus_mem1[AD_IND_DNS]=*(unsigned long int*)&buf[0];
if ( modbus_mem1[AD_IND_DNS] >99  | modbus_mem1[AD_IND_DNS] ==0 ) {WrArrayToFlesh(A_IND_DNS,0,2,1,0);modbus_mem1[AD_IND_DNS]=0;}

RdFromFleshToArr(A_IND_GZU,&buf[0],2);
modbus_mem1[AD_IND_GZU]=*(unsigned long int*)&buf[0];
if ( modbus_mem1[AD_IND_GZU] >54  | modbus_mem1[AD_IND_GZU] ==0 ) {WrArrayToFlesh(A_IND_GZU,0,2,1,0);modbus_mem1[AD_IND_GZU]=0;}
...
RdFromFleshToArr(A_SEG99,(unsigned char *)&c1_byte.mb[0],SEG99*2);  // параметры ГЗУ
...
{
RdFromFleshToArr(A_SEL_MODUL,&sel_modul,1);
}
...
// buf_tx_232[35]=0;              
  ...
  *(unsigned int*)&buf_tx_232[TR_CRC]=0;
  *(unsigned int*)&buf_tx_232[TR_CRC]=crc_m1(&buf_tx_232[TR_SRC],*(unsigned int*)&buf_tx_232[TR_LEN],0xffff);
...
else
{
   ...
   else for(i=0;i<8;i++) modbus_mem1[AD_TII1+i]=cnt_tii[i];  
  
  
if(simka>1)  {simka=SIM_RES; Fl_ch_sim=1;}  //dobavka
...
void monitor_beg_state_seq(unsigned char zad)
{
 ...
 if(zad==0)
 {
  ...
  if(sel_modul==1)
  {
     modbus_mem1[AD_TS]=modbus_mem1[AD_TS] | str_tc1.tek_pin | (str_tc2.tek_pin<<1) | (str_tc3.tek_pin<<2) | (str_tc4.tek_pin<<3)| (str_tc5.tek_pin<<4);
  }
  else
  {
  modbus_mem1[AD_TS]=modbus_mem1[AD_TS] | (str_tc2.tek_pin<<1) | (str_tc3.tek_pin<<2) | (str_tc4.tek_pin<<3)| (str_tc5.tek_pin<<4) | (str_tc6.tek_pin<<5) | (str_tc7.tek_pin<<6) | (str_tc8.tek_pin<<7);
  if(PINK & IO1)modbus_mem1[AD_TS]=modbus_mem1[AD_TS] & (~0x01); else modbus_mem1[AD_TS]=modbus_mem1[AD_TS] | 0x01;
  }
  */
...
if (Appl_seq_des==1)return;// блокировка сообщдений            
...
/*
            ...
            if (PINA & TCC2) {byte_state_net=NO_NET;send_info(sizeof(evc_feed_akb),evc_feed_akb,0,0);}
            else {byte_state_net=YES_NET;send_info(sizeof(evc_feed_220),evc_feed_220,0,0);}
...
else {sv2.tc_old=sv2.tc; buf[9]=DES_SEQ;}
...
{
if(fl_appl_seq1.send_state==1)fl_appl_seq1.send_state=0; 
buf[0]=SEQ_STATE;
*(unsigned int*)&buf[1]=L_STATE;
*(unsigned long int*)&buf[3]=burst_ds_r();
if(write_to_buf(&Appl_seq_buf[0],&Appl_seq.p_in,&Appl_seq.p_out,&Appl_seq.p_out_kv,&Appl_seq.crc,&buf[0],L_STATE+3, sizeof(Appl_seq_buf))==1)
...
void Application_SEQ_tx(void)
{
 ...
 if (fl_wdt.fl_out==1)
     ...
     if(*(unsigned int*)&keys[L_KEYS-2]!=crc_m1(&keys[0],L_KEYS-2,0xffff))
     {
               ...
               case STM_SC:
                              ...
                              switch(event_modem)
                                     ...
                                     case EVM_NONE: break;
                                      ...
                                      default: write_log_info(ST_ERROR,ERR8);
                                               lock_it();
                                               break; 
                             ...
                             if (layer_PPP!=LAYER_HW) 
                               ...
                               switch(event_modem)
                                     ...
                                     case EVM_PPP_OK:
                                                   ...
                                                   //!!!!!! dobavka
                                                   if((simka==SIM_RES)&&(cnt_link_res==0))cnt_link_res=vol_cnt_link_res;
                                                   //EEEEEEEEE dobavka
...
unsigned char run_cont_232_2 (unsigned char *pointer)
{
                ...
                switch ((*(pointer+1)) & 0x07)
                           {
                           case NON:  UCSR2C=UCSR2C & ~0x30;
                                 break;
                           case ODD:  UCSR2C=UCSR2C | 0x30;
                                 break;
                           case EVEN: UCSR2C=UCSR2C & ~0x30;
                                 UCSR2C=UCSR2C | 0x20;
                                 break;
                           default: return(1);
                           }
                 ...
                 switch (*pointer)
                            {
                              case B2400:
                                     ...
                                     if(Appl_RS232_2.pre_tx!=0)
                                        ...
                                        else Rs232_2.cnt_tm_pre_tx=17;// 17 миллисекунд
                              ...
                              case B4800:
                                     ...
                                     if(Appl_RS232_2.pre_tx!=0)
                                       ...
                                       //   __enable_interrupt();
                                        ...
                                        else Rs232_2.cnt_tm_pre_tx=9;// 9 миллисекунд
                              ...
                              case B9600:
                                     ...
                                     if(Appl_RS232_2.pre_tx!=0)
                                        ...
                                        else Rs232_2.cnt_tm_pre_tx=4;// 4 миллисекунд
                              ...
                              case B19200:
                                     ...
                                     if(Appl_RS232_2.pre_tx!=0)
                                        ...
                                        else Rs232_2.cnt_tm_pre_tx=2;// 2 миллисекунд
...
unsigned int proc_config(unsigned char *buf_rx_ppp,unsigned char offset,unsigned int count_rx_ppp)
 ...
 next_parametr: 
             ...
             case CONF_DES_OHR :  
                 ...
                 same_1:            ofs=TR_OP_DATA+C1_DATA+4+cnt_bt;
                                    temp=buf_tx_232[ofs];
               ...
               case CONF_RESET:          
                 ...
                 case CONF_PDP_R :                                                  
                                  if((buf_rx_ppp[offset+C1_DATA] & 0x80)==0x80)
                                    ...
                                    RdFromFleshToArr(A_CR_GPRS,&buf_tx_232[TR_OP_DATA+C1_DATA+4+cnt_bt],L_CR_GPRS_MAX);
                 ...
                 case CONF_PDP_DOP_R :                                                  
                                  if((buf_rx_ppp[offset+C1_DATA] & 0x80)==0x80)
                                    ...
                                    RdFromFleshToArr(ABUF_CR_GPRS,&buf_tx_232[TR_OP_DATA+C1_DATA+4+cnt_bt],temp); 
                                    buf_tx_232[TR_OP_DATA+C1_DATA+cnt_bt]=buf_rx_ppp[ind]; 
                                    *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+1+cnt_bt]=temp+1;                                  
                                    buf_tx_232[TR_OP_DATA+C1_DATA+3+cnt_bt]=0x00; //OK 
                 ...
                 case CONF_KEYS:
                                ...
                                if(*(unsigned int*)&buf_rx_ppp[ind+1]!=L_WR_CONF_KEYS)goto bad_com;   
                      ...
                      case CONF_TM_NO_LINK:
                        ...
                        if((buf_rx_ppp[offset+C1_DATA] & 0x80)==0x80)
                                    {
                                    if(*(unsigned int*)&buf_rx_ppp[ind+1]!=L_CONF_TM_NO_LINK)goto bad_com;
                                    //запись
                                    temp=*(unsigned int*)&buf_rx_ppp[ind+3];
                                    ...
                                    *(unsigned int*)&buf[OFS_C_PAR_CRC]=crc_m1(&buf[0],L_C_PAR-2,0xffff);
                                    ...
                                    else {if(*(unsigned int*)&buf_rx_ppp[ind+1]!=(L_CONF_TM_NO_LINK-2))goto bad_com;}
                                     

                                    buf_tx_232[TR_OP_DATA+C1_DATA+cnt_bt]=buf_rx_ppp[ind];  //чтение
                                    *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+1+cnt_bt]=L_CONF_TM_NO_LINK+1;   
                                    buf_tx_232[TR_OP_DATA+C1_DATA+3+cnt_bt]=0x00; //OK  
                                    RdFromFleshToArr(A_C_PAR+OFS_TM_NO_LINK,&buf_tx_232[TR_OP_DATA+C1_DATA+4+cnt_bt],2);
                       ...
                       case CONF_TM_LINK_RES:
                        ...
                        if((buf_rx_ppp[offset+C1_DATA] & 0x80)==0x80)
                                    {
                                    if(*(unsigned int*)&buf_rx_ppp[ind+1]!=L_CONF_TM_LINK_RES)goto bad_com;
                                    //запись
                                    temp=*(unsigned int*)&buf_rx_ppp[ind+3];
                                    ...
                                    *(unsigned int*)&buf[OFS_C_PAR_CRC]=crc_m1(&buf[0],L_C_PAR-2,0xffff);
                                    ...
                                    else {if(*(unsigned int*)&buf_rx_ppp[ind+1]!=(L_CONF_TM_LINK_RES-2))goto bad_com;}
                                     

                                    buf_tx_232[TR_OP_DATA+C1_DATA+cnt_bt]=buf_rx_ppp[ind];  //чтение
                                    *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+1+cnt_bt]=L_CONF_TM_LINK_RES+1;   
                                    buf_tx_232[TR_OP_DATA+C1_DATA+3+cnt_bt]=0x00; //OK  
                                    RdFromFleshToArr(A_C_PAR+OFS_TM_LINK_RES,&buf_tx_232[TR_OP_DATA+C1_DATA+4+cnt_bt],2);
                ...
                case CONF_TP_TS:   
                            if((buf_rx_ppp[offset+C1_DATA] & 0x80)==0x80)
                                    {
                                    if(*(unsigned int*)&buf_rx_ppp[ind+1]!=L_CONF_TP_TS)goto bad_com;
                                    //запись
                                    ...
                                    */
                                    ...
                                    else {if(*(unsigned int*)&buf_rx_ppp[ind+1]!=(L_CONF_TP_TS-2))goto bad_com;}
                                     

                                    buf_tx_232[TR_OP_DATA+C1_DATA+cnt_bt]=buf_rx_ppp[ind];  //чтение
                                    *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+1+cnt_bt]=L_CONF_TP_TS+1;   
                                    buf_tx_232[TR_OP_DATA+C1_DATA+3+cnt_bt]=0x00; //OK  
                                    RdFromFleshToArr(A_SEQ_PAR+OFS_TP_TS,&buf_tx_232[TR_OP_DATA+C1_DATA+4+cnt_bt],2);
                 ...
                 case CONF_TM_NAT_R:  
                                  ...
                                  if((buf_rx_ppp[offset+C1_DATA] & 0x80)==0x80)
                                    {
                                    if(*(unsigned int*)&buf_rx_ppp[ind+1]!=L_CONF_TM_NAT_R)goto bad_com;
                                    //запись
                                    temp=*(unsigned int*)&buf_rx_ppp[ind+3];
                                    ...
                                    *(unsigned int*)&buf[OFS_C_PAR_CRC]=crc_m1(&buf[0],L_C_PAR-2,0xffff);
                                    ...
                                    else {if(*(unsigned int*)&buf_rx_ppp[ind+1]!=(L_CONF_TM_NAT_R-2))goto bad_com;}
                                     

                                    buf_tx_232[TR_OP_DATA+C1_DATA+cnt_bt]=buf_rx_ppp[ind];  //чтение
                                    *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+1+cnt_bt]=L_CONF_TM_NAT_R+1;   
                                    buf_tx_232[TR_OP_DATA+C1_DATA+3+cnt_bt]=0x00; //OK  
                                    RdFromFleshToArr(A_C_PAR+OFS_NAT_R,&buf_tx_232[TR_OP_DATA+C1_DATA+4+cnt_bt],2);
                ...
                case CONF_TRAF :  
                                  ...
                                  if((buf_rx_ppp[offset+C1_DATA] & 0x80)==0x80)
                                    {
                                    if(*(unsigned int*)&buf_rx_ppp[ind+1]!=L_CONF_TRAF)goto bad_com;
                                    //запись
                                    ...
                                    *(unsigned int*)&buf[L_TRAF-2]=crc_m1(&buf[0],L_TRAF-2,0xffff);
                                    ...
                                    else {if(*(unsigned int*)&buf_rx_ppp[ind+1]!=L_CONF_TRAF)goto bad_com;}
                                     

                                    buf_tx_232[TR_OP_DATA+C1_DATA+cnt_bt]=buf_rx_ppp[ind];  //чтение
                                    *(unsigned int*)&buf_tx_232[TR_OP_DATA+C1_DATA+1+cnt_bt]=(L_TRAF-2)+1;   
                                    buf_tx_232[TR_OP_DATA+C1_DATA+3+cnt_bt]=0x00; //OK  
                                    RdFromFleshToArr(A_TRAF,&buf_tx_232[TR_OP_DATA+C1_DATA+4+cnt_bt],L_TRAF-2);
                  ...
                  case CONF_TEMPER :  
                                  ...
                                  if((buf_rx_ppp[offset+C1_DATA] & 0x80)==0x80)goto bad_com;
                                    ...
                                    else {if(*(unsigned int*)&buf_rx_ppp[ind+1]!=L_CONF_TEMPER)goto bad_com;}
...
void monitor1_tm_rs485_1(void)
  ...
  fl_485_1.tm_out=0; 
  if(PINE & DCD0) send_err485(NUM_RS485_1,RS_NO_LINK,Appl_RS485_1.id_tek,Appl_RS485_1.dst_tek);// игнорировать передачу если нет GPRS канала
  fl_485_1.busy=0; 
  fl_tx485.mon1=0;
  }
...
///EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE!Работа с портом RS485-1
...
void monitor1_tm_rs485_2(void)
  ...
  fl_485_2.tm_out=0; 
  if(PINE & DCD0) send_err485(NUM_RS485_2,RS_NO_LINK,Appl_RS485_2.id_tek,Appl_RS485_2.dst_tek);// игнорировать передачу если нет GPRS канала
  fl_485_2.busy=0; 
  fl_tx485.mon2=0;
  }
...
///EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE!Работа с портом RS485-2
...
int proc_temp_to_grad(char t,unsigned int t_cod,unsigned int offset_cod,unsigned char delta_t,unsigned char delta_cod)
 {
...
void control_temperatura(void)
{
  ...
  */
  if(temperatura>T_MINUS10)SET_TEN;
  if(temperatura<T_MINUS5)CLR_TEN;
