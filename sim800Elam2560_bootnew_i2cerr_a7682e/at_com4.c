...
__flash char stm_pin1_ok[]=        {'S','T','M',':','p','i','n',' ','s','i','m','1',' ','o','k'};
__flash char stm_pin2_ok[]=        {'S','T','M',':','p','i','n',' ','s','i','m','2',' ','o','k'};
__flash char stm_pin1_err[]=       {'S','T','M',':','p','i','n',' ','s','i','m','1',' ','e','r','r'};
__flash char stm_pin2_err[]=       {'S','T','M',':','p','i','n',' ','s','i','m','2',' ','e','r','r'};
...
{   
 if(At_com.cnt_tx>=(At_com.ln_buf-1))
   ...
   //  UCSR0B=UCSR0B & ~TXCIE; !!!!
    ...
    if(Regim==RG_DEBAG) UDR3=At_com.buf[At_com.cnt_tx];
} 
...
switch(At_com.list_com[cnt])
 ...
 //          strcpy(At_com.buf, "AT+CIMI");             // информация о SIM карте SIMI
 ...
 //          At_com.vol_tm_out=200;
           ...
           if(simka==SIM_BASE) RdFromFleshToArr(A_C_GPRS,&At_com.buf[18],LN_BUF_AT-19); //dobavka
             else RdFromFleshToArr(A_CR_GPRS,&At_com.buf[18],LN_BUF_AT-19);              //dobavka
           
           if(((unsigned char)At_com.buf[18]!=0)&&((unsigned char)At_com.buf[18]<31))
             {
   ...
   case cATCGATT0:
    ...
    case cATCPIN:
           ...
           if(simka==SIM_BASE) RdFromFleshToArr(A_PIN_SIM1,&At_com.buf[8],4); 
             else RdFromFleshToArr(A_PIN_SIM2,&At_com.buf[8],4);              
           At_com.buf[12]=0;
     ...
     case PLUS:
       ...
       //   fl_at_com.tm_out_en=1;
         ...
         //  UCSR0B=UCSR0B | RXCIE; //proverka !!!!!!!1
           ...
           S2_RD;
           UDR0=At_com.buf[0];
           if(Regim==RG_DEBAG) UDR3=At_com.buf[0];
...
switch(At_com.list_com[cnt])
    ...
    case cATCREG:
                  ...
                  //   if(At_com.buf[At_com.cnt_rx-2]=='2')fl_at_com.ok=1;  
                     if( (At_com.buf[At_com.cnt_rx-5]=='1')    || (At_com.buf[At_com.cnt_rx-5]=='5') ){fl_at_com.ok=1; state_led_md=LED_MD_YES_CREG;}   
                     else fl_at_com.err=1; 
           break;  
   ...
   case cATCEREG:  
                     if(At_com.buf[At_com.cnt_rx-1]!=0xd){fl_at_com.err=1;return;}
                  //   if(At_com.buf[At_com.cnt_rx-2]=='2')fl_at_com.ok=1;  
                     if ( (At_com.buf[At_com.cnt_rx-5]=='1') || (At_com.buf[At_com.cnt_rx-5]=='5') ) {fl_at_com.ok=1;state_led_md=LED_MD_GPRS;}    
                     else fl_at_com.err=1; 
           break;          
  ...
  //    while(*you <=0xd) you++;
    ...
    find_plus_:    
      //  emei_emei[0]=0;
      {
       ...
       for(i=0;i<At_com.cnt_rx;i++) {
         if(*you==0xd) {emei_emei[0]=i;return;}
         else emei_emei[i+1]=*you++;
       }
      }
...
*/   
 ...
 case cATCGREG:
           ...
           if(fl_at_com.ok==1){fl_at_com.greg_ereg =0; event_modem=EVM_AT_OK;return(1);}    // ОК режим GPRS
           if((fl_at_com.err==1)||(fl_at_com.tm_out==1))
           {
           (*rp)++;
           if(*rp>= 10 ){(*cnt)++;*rp=0;fl_at_com.tx_en=1;}      // пробуем режим LTE надо поставить 20
           fl_at_com.tx_en=1;
           }
       break;    
 ...
 case cATCEREG:      // LTE PD последняя в списке команд                   
           if(fl_at_com.ok==1){fl_at_com.greg_ereg =1; event_modem=EVM_AT_OK;return(1);}          //  ОК режим LTE
           if((fl_at_com.err==1)||(fl_at_com.tm_out==1))
           {
           (*rp)++;
           if(*rp>=20){event_modem=EVM_AT_ERR;return(3);}
...
*P++=cATCREG0;
 ...
 if(simka==SIM_BASE) RdFromFleshToArr(A_PIN_SIM1,&buf[0],4); 
                else RdFromFleshToArr(A_PIN_SIM2,&buf[0],4);              
if
...
//     (buf[3]>=0x30)&&(buf[3]<=0x39)){At_com.list_com[7]=cATCPIN;i=1; S5_RD;} else {S5_YL;}
    (buf[3]>=0x30)&&(buf[3]<=0x39)){*P++=cATCPIN; S5_RD;} else {S5_YL;}     
